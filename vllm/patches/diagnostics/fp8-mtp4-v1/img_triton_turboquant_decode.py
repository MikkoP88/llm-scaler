...
def _tq_mq_decode_stage1(
    ...
    Win_start_ptr,  # [B] int32: local window start for row 0 (v41; may be < 0)
    Centroids_ptr,  # [n_centroids] float32
    ...
    for start_n in range(split_start, split_end, BLOCK_KV):
        ...
        if KEY_FP8:
            ...
            if FP8_E4B15:
                ...
                k_float = k_raw.to(tl.float8e4nv, bitcast=True).to(tl.float32)
            # llm-scaler v32: per-row 2D score accumulation (register-lean).
            ...
            # llm-scaler v32: per-row 2D term1 (see KEY_FP8 site note).
        ...
        # ============================================================
        # llm-scaler v32: per-row 2D value accumulation (register-lean;
...
def triton_turboquant_mq_decode_attention(
    ...
    win_start: torch.Tensor | None = None,  # [B] int32 local window start (v41)
) -> torch.Tensor:
