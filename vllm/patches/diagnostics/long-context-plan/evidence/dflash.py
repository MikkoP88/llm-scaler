...
# llm-scaler v49c: the v21c tq4nc draft-dtype auto-policy (k8v4 above this
# len / "auto" below) is retired -- draft KV now defaults to matching
# --kv-cache-dtype for every dtype; VLLM_DFLASH_DRAFT_KV_DTYPE overrides.
# VRAM fact it encoded: an uncompressed (auto/bf16) draft pool next to a
# turboquant_4bit_nc target fits only up to ~133.6k tokens
# (6.67 GiB available / (13.09 GiB / 262144)); the certified prod boot pins
...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def _create_draft_vllm_config(self) -> VllmConfig:
        ...
        # llm-scaler v49c: draft KV dtype DEFAULTS TO MATCHING
        # --kv-cache-dtype (plain inherit for every target dtype); the
        # v21c auto-policy (tq4nc targets: k8v4 above 131072 / "auto"
        # below) is retired. The user-facing rule is now uniform --
        # fp8_e4m3 target -> fp8_e4m3 draft, turboquant_4bit_nc target ->
        # turboquant_4bit_nc draft, etc. -- and
        # VLLM_DFLASH_DRAFT_KV_DTYPE remains the single optional override
        # accepting any --kv-cache-dtype value (e.g. "auto",
        # "turboquant_k8v4", "turboquant_4bit_nc").
        # Migration notes (measured v21c facts the old policy encoded):
        #   * tq4nc @ >131072: an "auto" (bf16 flash) draft pool does NOT
        #     fit VRAM (13.09 GiB needed vs 6.67 GiB available at 262144).
        #     The certified prod cell is draft k8v4 (equal measured
        #     acceptance to a tq4nc draft, validated at 262144, 33.16
        #     tok/s steady vs 32.77-32.78 nospec) -- the prod boot pins
        #     VLLM_DFLASH_DRAFT_KV_DTYPE=turboquant_k8v4.
        #   * tq4nc @ <=131072: v21c defaulted the draft to "auto" (bf16
        #     flash pool: best acceptance E[len] ~2.0-2.4 vs ~1.78
        #     compressed; flash backend ~5-10 ms/step faster than TQ).
        #     Under v49c the default is now tq4nc; set the override to
        #     "auto" to restore that regime.
        #   * non-tq4nc targets already inherited (v20 behavior; fp8
        #     targets ran fp8 drafts, v20 c2 33.34 vs v21b 33.66 tok/s) --
        #     v49c changes nothing there.
        ...
        if draft_kv_override and draft_kv_override != base.cache_config.cache_dtype:
            logger.info(
                "DFlash draft KV dtype (v49c): draft pool uses %s while the "
                "target pool uses %s (max_model_len %d; "
                "VLLM_DFLASH_DRAFT_KV_DTYPE override; default is "
                "match-target).",
                draft_kv_override,
