#!/bin/bash
# t3i_chain.sh — boot df7t3i lane (v1.2.5t3i = t3f + v52j scheduler
# worst-case boundary clamp v4; v52h worker trim stays reverted).
# Validation targets:
#   - "v52j BOUNDARY-DEGRADE" / "v52j BOUNDARY-CLAMP" lines fire at
#     boundary zones (est/lo/prev)
#   - ZERO "v52d EMPTY-ROW" / ATTR all-NaN / F8v2 FORCE-FINISH (zombies)
#   - ZERO worker deaths (t3g native gather OOB must stay gone)
#   - all P1-P8 finish normally (streams crossing 2048 AND 4096)
LOG=/root/build/df7t3i_boot.out
{
...
nohup python3 /root/build/dt_loop8t07.py > /root/build/loop8t07_df7t3i.out 2>&1 &
