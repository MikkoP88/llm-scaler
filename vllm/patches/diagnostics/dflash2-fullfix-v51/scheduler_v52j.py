    ...
    def _mamba_block_aligned_split(
        ...
        if num_computed_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
            ...
            if (
                ...
                block_size = self.cache_config.block_size
                # llm-scaler v52i (crash-4 R1 ROOT FIX v3, 2026-09-06):
                # WORST-CASE room. t3f falsified the v52g est-based clamp
                # above: async scheduling plans this step while the worker
                # is still executing the previous one, so the CPU
                # num_computed_tokens is OPTIMISTIC by up to
                # (prev planned rows - 1) (t3f DEF-PP/PRE-COPY snapshots:
                # 8-row steps EXECUTED at actual 2046/2044 while clamps
                # engaged on estimates 2048+; zombie onset zone unchanged).
                # Bound the worst-case actual start position
                # lo = est - (prev_planned - 1) and clamp against lo's
                # room: even if the step starts at lo it stays inside one
                # mamba block.
                # NOTE v52h (t3g, worker-side row trim) is REVERTED and
                # must stay reverted: executing fewer rows than the engine
                # scheduled breaks async bookkeeping — the deferred
                # acceptance postprocess skips the request after the
                # mamba window moves, dflash propose() stalls (2.75s),
                # draft acceptance collapses to 0, -1 placeholder tokens
                # leak into the input window, and a vectorized gather
                # aborts the worker natively (IndexKernelUtils.h:63 OOB).
                # The scheduler side is the authoritative consistency-safe
                # clamp site: the engine plans exactly what the worker
                # executes (t3f/t3g proof: the v52g 5-row clamped step
                # 88059+5=88064 executed and promoted cleanly, acc=5).
                # v52j: worst-case containment, CORRECTED (v4). The v52i
                # comment above documents the async est-optimism; v52i's
                # arithmetic was wrong: room = bs - lo%bs binds at the
                # FARTHEST position from the boundary (max room) — a 7-row
                # step est=2048 lo=2041 executed at actual 2046..2052 and
                # SPANNED (t3h). Containment must bind at the position
                # CLOSEST to the boundary. With actual in [lo, est]:
                #   - if [lo, est] straddles a block boundary, only a
                #     1-row step is provably contained for every possible
                #     actual (worst case actual = boundary-1);
                #   - else (same block) clamp to bs - est%bs (binding at
                #     actual=est; v52g's arithmetic was right for this
                #     case, it only missed the straddle case).
                # gap model (data-verified twice): actual = est -
                # (prev_scheduled - prev_accepted), so
                # lo = est - (prev_scheduled - 1). The 1-row degrade is
                # the plain-decode shape the nospec lane executes at every
                # position; unlike the convicted v52h worker-side trim,
                # the engine schedules exactly what the worker executes.
                _v52j_prev = getattr(self, "_v52j_prev_new", None)
                if _v52j_prev is None:
                    _v52j_prev = self._v52j_prev_new = {}
                # Post-clamp SCHEDULED width of the previous decode-shaped
                # step: the phantom-row count bounding this step's gap.
                _v52j_prev_rows = _v52j_prev.get(request.request_id, 1)
                lo = max(
                    0,
                    num_computed_tokens - (_v52j_prev_rows - 1),
                )
                if lo // block_size != num_computed_tokens // block_size:
                    logger.warning(
                        "llm-scaler v52j BOUNDARY-DEGRADE: %s est=%d lo=%d "
                        "prev=%d num_new=%d blocksz=%d (possible start "
                        "positions straddle a mamba block boundary; only a "
                        "1-row step is provably contained)",
                        request.request_id,
                        num_computed_tokens,
                        lo,
                        _v52j_prev_rows,
                    ...
                    num_computed_tokens % block_size + num_new_tokens
                    > block_size
                ):
                    room = block_size - (
                    ...
                    logger.warning(
                        "llm-scaler v52j BOUNDARY-CLAMP: %s est=%d lo=%d "
                        "prev=%d num_new=%d room=%d blocksz=%d (worst-case "
                        "spec step would span mamba block boundary; "
                        "clamped)",
                        request.request_id,
                        num_computed_tokens,
                        lo,
                    ...
                    num_new_tokens = room
                _v52j_prev[request.request_id] = num_new_tokens
                if len(_v52j_prev) > 4096:
                    for _v52j_k in list(_v52j_prev):
                        if _v52j_k not in self.requests:
                            _v52j_prev.pop(_v52j_k, None)
