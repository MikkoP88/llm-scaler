...
class AsyncGPUModelRunnerOutput(AsyncModelRunnerOutput):
    # llm-scaler v52d: class-level episode dedup for the EMPTY-ROW probe
    # in get_output (a new wrapper instance is created every step, so
    # instance state would never persist).
    _v52d_seen: set = set()
    ...
    def get_output(self) -> ModelRunnerOutput:
        ...
        if max_gen_len == 1:
            ...
            # llm-scaler v52d (crash-4 R1 root-cause telemetry, spec path):
            # dump the RAW rejection-sampler row for requests whose parsed
            # emission is EMPTY while NOT discard-masked — the crash-4
            # zombie steady state (v52b: n=0 for 4+ consecutive steps;
            # v52c proved the discard mask is NEVER set for these).
            # Row taxonomy at onset: all -1 (PLACEHOLDER_TOKEN_ID) ->
            # sampler wrote explicit sentinels (verify skipped, row
            # touched); huge/garbage ints -> row NEVER written (request
            # outside the sampler's cu/idx coverage = batch-composition
            # desync); plausible token ids -> parse/mask mismatch.
            # Once per request per episode (class-level set: a new
            # wrapper instance is created every step). Detect-only.
            try:
                _inv = set(self._invalid_req_indices)
                _rraw = self.sampled_token_ids_cpu
                _rids = self._model_runner_output.req_ids
                for _i, _row in enumerate(valid_sampled_token_ids):
                    if _row or _i in _inv or _i >= _rraw.shape[0]:
                        continue
                    _rid = _rids[_i] if _i < len(_rids) else f"idx{_i}"
                    if _rid in AsyncGPUModelRunnerOutput._v52d_seen:
                        continue
                    AsyncGPUModelRunnerOutput._v52d_seen.add(_rid)
                    if len(AsyncGPUModelRunnerOutput._v52d_seen) > 256:
                        AsyncGPUModelRunnerOutput._v52d_seen.clear()
                    logger.warning(
                        "llm-scaler v52d EMPTY-ROW: %s slot=%d raw=%s "
                        "max_gen_len=%d invalid=%s — crash-4 zombie "
                        "emission site (R1 target)",
                        _rid, _i, _rraw[_i].tolist(), max_gen_len,
                        sorted(_inv) if _inv else "none")
                    # llm-scaler v52e (crash-4 R1 attribution): one sync'd
                    # dump per zombie episode — decides between:
                    #   (a) n_draft==0: scheduler stopped attaching drafts
                    #       (async spec desync) -> bonus-only row;
                    #   (b) stale greedy slot: greedy kernel wrote argmax
                    #       over a degenerate logits row at pos 0;
                    #   (c) draft row containing >=vocab ids (input_ids
                    #       garbage at logits_indices).
                    try:
                        _refs = getattr(self, "_v52e_refs", None)
                        if _refs is not None:
                            _meta, _lg, _bn, _tg, _temp = _refs
                            _ndt = list(_meta.num_draft_tokens)
                            _cu = _meta.cu_num_draft_tokens.detach().cpu().tolist()
                            _s = _cu[_i - 1] if _i else 0
                            _e = _cu[_i] if _i < len(_cu) else _s
                            _dt = getattr(_meta, "draft_token_ids", None)
                            _drow = _dt.detach()[_s:_e].cpu().tolist() if _dt is not None else None
                            _bli = _meta.bonus_logits_indices.detach().cpu().tolist()
                            _tli = _meta.target_logits_indices.detach().cpu().tolist()
                            _li = _meta.logits_indices.detach().cpu().tolist()
                            _bnv = _bn.detach().flatten().cpu().tolist() if _bn is not None else None
                            _tv = _temp.detach().cpu().tolist() if _temp is not None else None
                            _tam = (_tg.detach().float().argmax(-1).cpu().tolist()
                                    if _tg is not None else None)
                            _brow = None
                            if (_lg is not None and _i < len(_bli)
                                    and 0 <= _bli[_i] < _lg.shape[0]):
                                _r = _lg[_bli[_i]].detach().float()
                                _brow = (float(_r.min()), float(_r.max()),
                                         int(_r.argmax()))
                            logger.warning(
                                "llm-scaler v52e ATTR: %s slot=%d n_draft=%d "
                                "batch_n=%d cu=%s draftrow=%s bonus=%s "
                                "temp[slot]=%s bli[slot]=%s targ_am=%s "
                                "brow(min,max,am)=%s logit_rows=%d li=%s",
                                _rid, _i,
                                _ndt[_i] if _i < len(_ndt) else -1,
                                len(_ndt), _cu[:12],
                                _drow[:8] if _drow is not None else None,
                                _bnv[_i] if _bnv and _i < len(_bnv) else None,
                                _tv[_i] if _tv and _i < len(_tv) else None,
                                _bli[_i] if _i < len(_bli) else None,
                                _tam[_s:_e] if _tam is not None else None,
                                _brow,
                                _lg.shape[0] if _lg is not None else -1,
                                _li[:16])
                            # llm-scaler v52f (crash-4 R1 onset probe):
                            # worker counters + mamba state idx + accepted
                            # history + the token window around the
                            # optimistic gather position. A vocab_size
                            # entry inside tokwin means the gather read the
                            # pad (embedding OOB -> NaN hiddens); a bias in
                            # PRE-COPY larger than the last acc_hist entry
                            # means the boundary carry read an unwritten
                            # GDN state slot.
                            try:
                                _nct, _ntns, _midx, _hist = getattr(
                                    self, "_v52f_ctx",
                                    (None, None, None, None))
                                _ib = getattr(self, "_v52f_ib", None)
                                _tokwin = _istate = None
                                if (_ib is not None and _nct is not None
                                        and _i < len(_nct)):
                                    _nc = int(_nct[_i])
                                    _s0 = max(0, _nc - 8)
                                    _tokwin = _ib.token_ids_cpu[
                                        _i, _s0:_nc + 16].tolist()
                                    _istate = _ib.is_token_ids[
                                        _i, _s0:_nc + 16].tolist()
                                logger.warning(
                                    "llm-scaler v52f CTX: %s slot=%d nct=%s "
                                    "ntns=%s midx=%s acc_hist=%s "
                                    "tokwin=%s istok=%s",
                                    _rid, _i,
                                    int(_nct[_i]) if _nct is not None and _i < len(_nct) else None,
                                    int(_ntns[_i]) if _ntns is not None and _i < len(_ntns) else None,
                                    _midx.get(_rid) if _midx else None,
                                    _hist.get(_rid) if _hist else None,
                                    _tokwin, _istate)
                            except Exception as _ex:
                                logger.warning(
                                    "llm-scaler v52f CTX failed: %r", _ex)
                    except Exception as _ex:
                        logger.warning("llm-scaler v52e ATTR failed: %r", _ex)
            except Exception:
                pass
...
class GPUModelRunner(
    ...
    def _prepare_inputs(
        ...
        # llm-scaler v52f (crash-4 R1 onset probe): the gather above reads
        # token_ids_cpu at OPTIMISTIC positions (num_computed + 0..k). If a
        # row lands beyond the last WRITTEN token (real + drafts) it reads
        # the vocab_size pad -> embedding gather out-of-range -> NaN
        # hiddens -> the v52e all-NaN logits. CPU-side check on the
        # gathered ids; no GPU sync, n_scheduled comparisons per step.
        # Throttled: warn at most once per 5 s so a latched zombie cannot
        # flood the log.
        try:
            _gin = self.input_ids.cpu[:total_num_scheduled_tokens]
            _bad = (_gin >= self.input_batch.vocab_size)
            if bool(_bad.any()):
                import time as _v52f_time
                _now = _v52f_time.monotonic()
                if (_now - getattr(self, "_v52f_oob_last", 0.0)) > 5.0:
                    self._v52f_oob_last = _now
                    _rows = torch.nonzero(_bad).flatten().tolist()
                    logger.warning(
                        "llm-scaler v52f INPUT-OOB: %d/%d gathered ids "
                        ">= vocab (first rows=%s ids=%s vocab=%d) — "
                        "optimistic gather read past the written token "
                        "window (embedding OOB -> NaN hiddens)",
                        len(_rows), total_num_scheduled_tokens,
                        _rows[:8],
                        [int(_gin[r]) for r in _rows[:8]],
                        self.input_batch.vocab_size)
        except Exception:
            pass
        if self.enable_prompt_embeds:
        ...
        # llm-scaler v52c (crash-4 R1 root-cause telemetry): log the
        # worker-vs-scheduler token-count gap that drives the discard
        # mask, once per (request, gap) episode. In the crash-4 zombie
        # the mask latches True permanently: the discarded request emits
        # nothing, scheduler-side num_computed_tokens never advances,
        # and the optimistic estimate can never bridge the gap. This
        # probe answers R1 (the still-open trigger): the gap VALUE at
        # first onset and whether it jumps in a single step (single
        # desync event, e.g. an APC 2048-token block boundary — both
        # observed onsets were just past a 2048 multiple: 2085, 4172)
        # or accumulates gradually. Detect-only; wrapped so telemetry
        # can never take the engine down.
        try:
            _mask = self.discard_request_mask.np[:num_reqs]
            _disc = np.nonzero(_mask)[0]
            if _disc.size:
                _gaps = (num_tokens_np[:num_reqs]
                         - self.optimistic_seq_lens_cpu[:num_reqs].numpy())
                _seen = getattr(self, "_v52c_seen", None)
                if _seen is None:
                    _seen = self._v52c_seen = {}
                for _i in _disc:
                    _i = int(_i)
                    _rid2 = self.input_batch.req_ids[_i]
                    _gap = int(_gaps[_i])
                    if _seen.get(_rid2) == _gap:
                        continue
                    _seen[_rid2] = _gap
                    logger.warning(
                        "llm-scaler v52c DISCARD-GAP: %s slot=%d "
                        "gap=%d (worker num_tokens=%d > optimistic "
                        "computed+scheduled=%d) — request discarded "
                        "this step; a latching/growing gap is the "
                        "crash-4 zombie onset (R1 target)",
                        _rid2, _i, _gap, int(num_tokens_np[_i]),
                        int(self.optimistic_seq_lens_cpu[_i]))
                if len(_seen) > 4 * num_reqs + 64:
                    _live = set(self.input_batch.req_ids)
                    for _k in list(_seen):
                        if _k not in _live:
                            _seen.pop(_k, None)
        except Exception:
            pass
        ...
        if self.num_accepted_tokens_event is not None:
            ...
            # llm-scaler v52f: rolling per-request accepted-token history
            # (last 6 steps) for the v52d event dump — preprocess_mamba's
            # 2048-boundary carry bias is num_accepted_tokens_cpu - 1; a
            # bias larger than the true previous-step accepted count reads
            # an unwritten GDN state slot (garbage -> NaN poison).
            try:
                _hist = getattr(self, "_v52f_acc_hist", None)
                if _hist is None:
                    _hist = self._v52f_acc_hist = {}
                _na = self.num_accepted_tokens.np
                for _hi in range(num_reqs):
                    _hrid = self.input_batch.req_ids[_hi]
                    _h = _hist.get(_hrid)
                    if _h is None:
                        _h = _hist[_hrid] = []
                    _h.append(int(_na[_hi]))
                    del _h[:-6]
                if len(_hist) > 4096:
                    _live = set(self.input_batch.req_ids)
                    for _hk in list(_hist):
                        if _hk not in _live:
                            _hist.pop(_hk, None)
            except Exception:
                pass
    ...
    def _sample(
        ...
        # llm-scaler v52e (crash-4 R1 attribution): keep per-step refs so
        # get_output() can attribute an empty row to n_draft / bonus /
        # temperature-slot / logits-row degeneracy at the moment it is
        # observed (refs only; no copies, no syncs on the hot path).
        self._v52e_temp = sampling_metadata.temperature
    ...
    def execute_model(
        ...
        with (
            ...
            # llm-scaler v52h (crash-4 R1 ROOT FIX v2, 2026-09-06):
            # worker-side authoritative mamba block-boundary clamp.
            #
            # The scheduler-side v52g clamp is NECESSARY BUT NOT
            # SUFFICIENT in async-scheduling mode: the scheduler plans
            # step N+1 at an OPTIMISTIC computed estimate while the
            # worker executes the rows at the corrected ACTUAL position
            # (= estimate - shortfall, shortfall = prev step's
            # placeholder optimism - acceptance, up to prev rows - 1).
            # t3f live proof: scheduler clamped at estimates
            # 2041..2045 yet DEF-PP snapshots show 8-row steps EXECUTED
            # at actual 2046/2044 -> spanning -> all-NaN target logits
            # (v52e) -> zombie emission (v52d [248320, -1 x7]).
            #
            # Here the row counts are trimmed with a worst-case-safe
            # rule BEFORE the input gather, using the same
            # num_computed_tokens_cpu the gather positions derive from:
            #   lo = est - (prev_sched_rows - 1)  (min possible actual)
            #   1) lo and est straddle a block boundary -> only 1 row is
            #      provably contained for every position in [lo, est];
            #      degrade the step to a plain decode (no drafts).
            #   2) est % bs + rows > bs -> clamp rows to room at est
            #      (positions below est only have MORE room).
            # Executed-width reduction is reconciled downstream by the
            # existing async correction machinery (identical to a
            # natural acceptance shortfall). Scheduler v52g clamp stays
            # as belt-and-braces (it only shrinks; never widens).
            _v52h_bs = getattr(self, "_v52h_bs", None)
            if _v52h_bs is None:
                _v52h_bs = 0
                try:
                    from vllm.v1.worker.mamba_utils import (
                        get_mamba_groups as _v52h_gmg,
                    )
                    _v52h_bs = _v52h_gmg(self.kv_cache_config)[1].block_size
                except Exception:
                    _v52h_bs = 0
                self._v52h_bs = _v52h_bs
            if _v52h_bs:
                _v52h_spec = scheduler_output.scheduled_spec_decode_tokens
                _v52h_prev = getattr(self, "_v52h_prev_rows", None)
                if _v52h_prev is None:
                    _v52h_prev = self._v52h_prev_rows = {}
                _v52h_nct = self.input_batch.num_computed_tokens_cpu
                _v52h_trimmed = 0
                for _v52h_j, _v52h_rid in enumerate(req_ids):
                    _v52h_rows = tokens[_v52h_j]
                    _v52h_dl = _v52h_spec.get(_v52h_rid)
                    if _v52h_rows <= 1 or not _v52h_dl:
                        # plain decode / prefill: no placeholder optimism
                        # can shift the next step's position.
                        _v52h_prev[_v52h_rid] = 1
                        continue
                    _v52h_hi = int(_v52h_nct[_v52h_j])
                    # positions are never negative; clamp the worst-case
                    # low bound so young requests (early decode) don't
                    # straddle-test against block -1 spuriously.
                    _v52h_lo = max(
                        0,
                        _v52h_hi - (_v52h_prev.get(_v52h_rid, 1) - 1))
                    _v52h_new = _v52h_rows
                    if _v52h_lo // _v52h_bs != _v52h_hi // _v52h_bs:
                        _v52h_new = 1
                        logger.warning(
                            "llm-scaler v52h EXEC-DEGRADE: %s est=%d lo=%d "
                            "rows=%d bs=%d (async shortfall may position "
                            "this step across a mamba block boundary; "
                            "degraded to 1 row)",
                            _v52h_rid, _v52h_hi, _v52h_lo, _v52h_rows,
                            _v52h_bs)
                    elif (_v52h_hi % _v52h_bs) + _v52h_rows > _v52h_bs:
                        _v52h_new = _v52h_bs - (_v52h_hi % _v52h_bs)
                        logger.warning(
                            "llm-scaler v52h EXEC-CLAMP: %s est=%d rows=%d "
                            "room=%d bs=%d (spec step would span mamba "
                            "block boundary; clamped)",
                            _v52h_rid, _v52h_hi, _v52h_rows, _v52h_new,
                            _v52h_bs)
                    if _v52h_new < _v52h_rows:
                        tokens[_v52h_j] = _v52h_new
                        _v52h_spec[_v52h_rid] = _v52h_dl[: _v52h_new - 1]
                        _v52h_trimmed += _v52h_rows - _v52h_new
                    # Record the SCHEDULER's original row count: the
                    # placeholder optimism bounding the NEXT step's
                    # shortfall is (original rows - acceptance).
                    _v52h_prev[_v52h_rid] = _v52h_rows
                if len(_v52h_prev) > 4096:
                    _v52h_live = set(req_ids)
                    for _v52h_k in list(_v52h_prev):
                        if _v52h_k not in _v52h_live:
                            _v52h_prev.pop(_v52h_k, None)
                if _v52h_trimmed:
                    scheduler_output.total_num_scheduled_tokens -= _v52h_trimmed
    ...
    def sample_tokens(
        ...
        with record_function_or_nullcontext(
            ...
            # llm-scaler v52e: attach step-accurate refs for the empty-row
            # attribution dump in get_output() (refs only, per-step object).
            async_output._v52e_refs = (
                spec_decode_metadata,
                logits,
                getattr(self.rejection_sampler, "_v52e_bonus", None),
                getattr(self.rejection_sampler, "_v52e_targ", None),
                getattr(self, "_v52e_temp", None),
            )
            # llm-scaler v52f (crash-4 R1 onset probe): snapshot the worker
            # counters that drive (a) the optimistic input gather and (b)
            # preprocess_mamba's boundary carry bias, plus a ref to the
            # input batch for the token window. Counters are COPIES (they
            # mutate in place); the input_batch ref is safe (one-step
            # object lifetime, runner-owned).
            try:
                _nr = self.input_batch.num_reqs
                async_output._v52f_ctx = (
                    self.input_batch.num_computed_tokens_cpu[:_nr].copy(),
                    self.input_batch.num_tokens_no_spec[:_nr].copy(),
                    dict(self.mamba_state_idx),
                    getattr(self, "_v52f_acc_hist", {}),
                )
                async_output._v52f_ib = self.input_batch
            except Exception:
                pass
        with record_function_or_nullcontext(
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(kv_cache_spec, AttentionSpec):
                    ...
                    if kv_cache_spec.page_size_padded is not None:
                        ...
                        # Page-size unification can grow block_size (e.g. a
                        # TurboQuant target page raised to match GDN pages),
                        # so one padded page spans num_blocks_per_kv_block
                        # kernel blocks. Give each kernel block an equal
                        # share of the padded page: using the full padded
                        # page as the per-block stride overflows the
                        # allocation by exactly that factor (dflash +
                        # turboquant_4bit_nc crash: setStorage wanting 16x
                        # the allocated storage).
                        if num_blocks_per_kv_block > 1:
                            assert page_stride % num_blocks_per_kv_block == 0, (
                                "padded KV page not divisible by kernel "
                                "blocks per page"
                            )
                            page_stride //= num_blocks_per_kv_block
                        strides = list(torch.empty(kv_cache_shape).stride())
                        if kv_cache_shape[inv_order[0]] == 2:
                            # K/V-first layout, e.g. (2, num_blocks, ...):
                            # each block's slot holds K then V adjacently, so
                            # the K/V dim strides by one real K half and the
                            # block dim strides by the padded page size.
                            kv_half_stride = strides[inv_order[1]]
                            strides[inv_order[0]] = kv_half_stride
                            strides[inv_order[1]] = page_stride
                        else:
                            # Block-first layout (e.g. MLA): the first
                            # dimension is the block index.
    ...
    def _update_hybrid_attention_mamba_layout(
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                # With a padded page, blocks stride by the padded page size
                # instead of 2 * hidden_size so the per-page tail padding is
                # skipped (must match _reshape_kv_cache_tensors).
                block_stride = 2 * hidden_size
                if kv_cache_spec.page_size_padded is not None:
                    block_stride = (
