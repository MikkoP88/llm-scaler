...
class DFlash2Proposer(DFlashProposer):
    def __init__(
        ...
        # llm-scaler v50: default flipped ON -> OFF. The v48 replication
        # lane is CONVICTED CORRUPT (acceptance 2-10%, per-position rates
        # ~0.2/0.02/0.00 vs ~0.59/0.34/0.16 stock) and was the necessary
        # co-factor of the fatal block_hashes assert (R2/R3 2026-09-05):
        # its draft-spec remap (2048->1024) desynced the request hasher
        # granularity from the pool. v50 core.py resync + block_pool.py
        # clamp make the engine survive this lane; this flip keeps it out
        # of prod. Opt back in at your peril.
        _v50_tp1 = os.environ.get("VLLM_XPU_DFLASH_TP1", "0") not in (
            "0",
            "false",
            "no",
            "off",
        ...
        if _v50_tp1:
            logger.warning(
                "DFlash2Proposer v50: drafter TP=1 replication EXPLICITLY "
                "ENABLED (VLLM_XPU_DFLASH_TP1) — CONVICTED CORRUPT lane "
                "(v48: acceptance 2-10%, per-position ~0.2/0.0/0.0; "
                "default OFF since v50). Output quality is degenerate."
            )
        logger.info(
            "DFlash2Proposer v48: drafter TP=1 replication %s "
            "(VLLM_XPU_DFLASH_TP1; default OFF since v50)",
            "ON" if _v50_tp1 else "OFF",
        )
    ...
    def _create_draft_vllm_config(self):
        ...
        if os.environ.get("VLLM_XPU_DFLASH_TP1", "0") in (
            "0",
            "false",
            "no",
            "off",
