    ...
    def _mamba_block_aligned_split(
        ...
        if num_computed_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
            ...
            if (
                ...
                _v52i_prev = getattr(self, "_v52i_prev_new", None)
                if _v52i_prev is None:
                    _v52i_prev = self._v52i_prev_new = {}
                # Record the ORIGINAL planned width (pre-clamp): the
                # placeholder optimism bounding the next step's shortfall
                # is (planned - acceptance), worst case planned - 1.
                _v52i_prev_rows = _v52i_prev.get(request.request_id, 1)
                _v52i_prev[request.request_id] = num_new_tokens
                if len(_v52i_prev) > 4096:
                    for _v52i_k in list(_v52i_prev):
                        if _v52i_k not in self.requests:
                            _v52i_prev.pop(_v52i_k, None)
                lo = max(
                    0,
                    num_computed_tokens - (_v52i_prev_rows - 1),
                )
                room = block_size - (lo % block_size)
                if num_new_tokens > room:
                    logger.warning(
                        "llm-scaler v52i BOUNDARY-CLAMP: %s est=%d lo=%d "
