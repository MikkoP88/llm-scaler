    ...
    def _mamba_block_aligned_split(
        ...
        if num_computed_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
            ...
            if (
                ...
                block_size = self.cache_config.block_size
                room = block_size - (num_computed_tokens % block_size)
                if num_new_tokens > room:
                    logger.warning(
                        "llm-scaler v52g BOUNDARY-CLAMP: %s computed=%d "
                        "num_new=%d room=%d blocksz=%d (spec decode step "
                        "would span mamba block boundary; clamped)",
                        request.request_id,
                        num_computed_tokens,
                        num_new_tokens,
                        room,
