...
# llm-scaler v52g (crash-4 hardening): INPUT-SANITIZE — clamp negative
# (-1 zombie draft placeholder) token ids to 0 in the assembled input
# window in _prepare_inputs, preventing the native embedding-gather OOB
# (IndexKernelUtils.h:63) that killed BOTH TP workers.
...
class GPUModelRunner(
    ...
    def _prepare_inputs(
        ...
        try:
            ...
            pass
        # llm-scaler v52g INPUT-SANITIZE: zombie requests leak -1
        # placeholder drafts into the input window when drafts go
        # missing (all-NaN logits path); the embedding gather dies
        # natively (IndexKernelUtils.h:63) and kills BOTH workers.
        # Clamp to token 0 — the row is pathological anyway; this
        # keeps the engine alive so F8v2 can reap the request.
        # Host-side site (CPU pin buffer, pre-upload; no device
        # sync) — last write before copy_to_gpu / the forward.
        _neg = int((self.input_ids.cpu[:total_num_scheduled_tokens] < 0).sum())
        if _neg:
            logger.warning(
                "llm-scaler v52g INPUT-SANITIZE: %d negative token ids "
                "clamped to 0 before forward", _neg)
