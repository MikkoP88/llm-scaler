...
class OpenAIServingChat(OpenAIServing):
    ...
    async def chat_completion_stream_generator(
        ...
        try:
            ...
            # llm-scaler v51 (F9 serving companion): deterministically close
            # the engine generator. The except branches above end this SSE
            # generator with a normal return, which previously left
            # result_generator suspended mid-yield with no CancelledError /
            # GeneratorExit ever delivered — the crash-3 zombie leak. With
            # F9-core (async_llm) the close delivers GeneratorExit and the
            # engine request is aborted immediately. aclose() on an already
            # exhausted generator is a no-op.
            aclose = getattr(result_generator, "aclose", None)
