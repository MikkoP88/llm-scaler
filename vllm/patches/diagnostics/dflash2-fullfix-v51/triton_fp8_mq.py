...
Knobs (env):
  ...
  VLLM_FP8MQ_BLOCK_KV      KV tile width, default 32.
  VLLM_FP8MQ_SPLITS        kv split count, default 32. Server A/B
                             (2026-09-06, mtp4+e4m3, 256-tok decode):
                             SPLITS=32 = 73.7/32.3/27.2/22.3 tps at
                             2k/16k/32k/65k ctx (parity with tq4nc
                             incumbent 72.0/38.1/27.6/21.5 — default keeps
                             the "no 2k regression" gate: +2.4% at 2k).
                             SPLITS=64 = 70.5/37/37.5/25.7 (long-ctx win:
                             +34% @32k, +17% @65k, -2% @2k) — recommended
                             env for long-context deployments.
                             SPLITS=128 = DO NOT USE: hard wedge, #11-class
                             crawl (0.2-0.3 tok/s, metrics log freezes,
                             survives client kill; grid B*Hq*128 +
                             128-iteration stage-2 combine trips the
                             collective-starvation path). 64 is the
                             certified ceiling.
  VLLM_FP8MQ_STAGE1_WARPS  default 4 (2026-09-06 XPU sweep at B=8/q5/
                             Hq=32/D=256/ctx=4096: W4 8.8 ms vs W1 20.6 ms
                             = 2.35x; patch-08's W1-fastest finding does
                             NOT transfer to this kernel on B70).
...
_V51_FP8MQ_SPLITS = max(1, int(os.environ.get("VLLM_FP8MQ_SPLITS", "32") or 32))
