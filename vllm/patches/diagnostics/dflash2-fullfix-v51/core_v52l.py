...
# llm-scaler v52: patch history in this file: v52 (crash-4 R2 root fix:
# flush pending scheduler-side finishes on idle/non-executed steps — see
# _v52_flush_pending_finishes and the idle-loop comment in
# _process_input_queue).
# v52l: EMPTY-SKIP — 0-token SchedulerOutputs short-circuit before execute_model (workers assert total>0); enables true scheduler-side boundary skips.
...
class EngineCore:
    ...
    def __init__(
        ...
        if vllm_config.cache_config.enable_prefix_caching or kv_connector is not None:
            ...
            # llm-scaler v51 (F2a ROOT FIX): the hasher closure is NO
            # LONGER built here from the resolve_kv_cache_block_sizes
            # value — the v50 code built it early and then conditionally
            # re-synced below (plaster). v51 builds it ONCE, below, from
            # the FINAL pool hash granularity. Safe to defer: the only
            # consumer is Request.from_engine_core_request at request-add
            # time (line ~844); nothing between the two sites reads it —
            # requests do not exist yet.

        # llm-scaler v51 (F2a ROOT FIX, replaces the v50 re-sync plaster):
        # construct the request block hasher ONCE from the FINAL pool hash
        # granularity. The scheduler + block pool were constructed above
        # from the post-remap group specs; a hasher built from the raw
        # resolve_kv_cache_block_sizes value can be coarser (2048 vs the
        # pool's final 1024 under VLLM_XPU_DFLASH_TP1's dflash2 v48
        # draft-spec remap), and a coarser hasher starves
        # cache_full_blocks of hashes -> fatal assert block_pool.py:240
        # (crash R2/R3, 2026-09-05). v50 patched this up after the fact
        # (rebuild only in the starving direction); v51 is
        # correct-by-construction and needs no conditional repair.
        _v51_pool_hbs = getattr(
            getattr(
                getattr(self.scheduler, "kv_cache_manager", None),
                "block_pool",
                None,
            ),
            "hash_block_size",
        ...
        _v51_hbs = _v51_pool_hbs if _v51_pool_hbs is not None else hash_block_size
        if (
            vllm_config.cache_config.enable_prefix_caching
            or kv_connector is not None
        ):
            self.request_block_hasher = get_request_block_hasher(
                _v51_hbs, caching_hash_fn
            )
            if self.use_spec_decode:
                logger.info(
                    "llm-scaler v51: block hasher built from final pool "
                    "granularity hbs=%s (pool=%s resolve=%s)",
                    _v51_hbs,
                    _v51_pool_hbs,
                    hash_block_size,
                )

        # llm-scaler v51: the v50 F6 refusal (fp8-class KV + spec decode
        # refused at boot) is REMOVED — root-fixed by the v51 dedicated
        # Triton fp8 multi-query flash-decode kernel
        # (vllm/v1/attention/ops/triton_fp8_mq.py, routed in
        # flash_attn.py before the v33 unified_attention path). The B5b
        # 5 s/step conviction was measured with the v33 route active;
        # see dflash2-fullfix-v51/PLAN.md Phase 1.
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        if scheduler_output.total_num_scheduled_tokens == 0:
            # llm-scaler v52l EMPTY-SKIP: zero-token step (all requests
            # boundary-skipped by the v52k scheduler or near-max-skipped).
            # Never dispatch to the workers: _prepare_inputs asserts
            # total_num_scheduled_tokens > 0 and kills BOTH TP workers (the
            # documented crash-4 incident). The engine busy-waits (the 1ms
            # not-model_executed sleep) until acceptance reports land and
            # the scheduler produces real work.
            return {}, False
...
class EngineCoreProc(EngineCore):
    ...
    def _process_input_queue(self):
        ...
        while not self.has_work() and self.is_running():
            # llm-scaler v52 (crash-4 R2 root fix): the engine is about
            # to park. Flush any scheduler-side pending request
            # terminations (e.g. F8v2 force-finish FINISHED_ABORTED) to
            # the client NOW: in async scheduling every delivery path for
            # a finished request's final output runs inside
            # update_from_output, which requires an executed step — if the
            # reaped request was the last one in flight, no further step
            # ever happens and the client stream would hang forever (the
            # crash-3 "usage-less silent stream end", reproduced
            # 2026-09-06 16:14 on v1.2.5t3).
            self._v52_flush_pending_finishes()
    ...
    def _v52_flush_pending_finishes(self) -> None:
        """llm-scaler v52 (crash-4 R2 root fix): drain scheduler-side
        pending request terminations into the output queue.

        AsyncScheduler.flush_pending_finishes() returns terminal
        EngineCoreOutputs entries (empty token list + finish_reason) for
        requests the scheduler force-finished while they were NOT part
        of a scheduled step (F8v2 zombie reaping) — the one finish class
        that otherwise has no delivery path in async scheduling.
        getattr-guarded so non-v52 scheduler objects are unaffected.
        """
        _flush = getattr(self.scheduler, "flush_pending_finishes", None)
        if _flush is None:
            return
        pending = _flush()
        for out in pending.items() if pending else ():
    ...
    def _process_engine_step(self) -> bool:
        ...
        # llm-scaler v52 (crash-4 R2 root fix): also flush pending
        # terminations on steps that did not execute the model (idle
        # with unfinished requests): update_from_output did not run, so
        # F8v2 reaps would strand; see the idle-loop comment above.
        self._v52_flush_pending_finishes()
