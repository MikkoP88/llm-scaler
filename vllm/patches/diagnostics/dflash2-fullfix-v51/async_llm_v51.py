...
class AsyncLLM(EngineClient):
    ...
    async def generate(
        ...
        *,
        ...
        # llm-scaler v51 (F9): close the crash-3 leak class. generate()
        # could exit without any abort when the caller stopped iterating
        # without cancelling (serving-layer exception swallowed upstream,
        # wrapper-generator early return, GC-deferred aclose) — the request
        # then kept decoding server-side until unresolved output
        # placeholders drove a fatal worker-side gather OOB. Track clean
        # completion vs explicit abort; finally aborts on every other exit.
        ...
        # we abort the request if we end up here.
            ...
            # llm-scaler v51 (F9): do NOT await here. Under re-delivered
            # cancellation the await can be cut mid-abort (front-end state
            # cleaned while EngineCore still schedules the request), and an
            # await under GC-driven GeneratorExit raises RuntimeError
            # ("async generator ignored GeneratorExit"). Fire-and-forget.
            if q is not None:
                abort_initiated = True
                try:
                    asyncio.get_running_loop().create_task(
                        self.abort(q.request_id, internal=True))
                except RuntimeError:
                    pass
            if self.log_requests:
        ...
        # Unexpected error in the generate() task (possibly recoverable).
            ...
            if q is not None:
                # llm-scaler v51 (F9): abort on ANY exit path that is not a
                # clean finish and did not already abort — covers the
                # serving-layer leak paths (exception swallowed upstream,
                # normal return of a wrapper generator, deferred GC close).
                # abort() is idempotent at both tiers (OutputProcessor
                # pop-noop; EngineCore abort is dead-safe).
                if not finished_cleanly and not abort_initiated:
                    abort_initiated = True
                    try:
                        asyncio.get_running_loop().create_task(
                            self.abort(q.request_id, internal=True))
                        if self.log_requests:
                            logger.info(
                                "Request %s aborted (F9: generate() exited "
                                "without finish or abort).", request_id)
                    except RuntimeError:
                        pass
                q.close()
    ...
    async def encode(
        ...
        q: RequestOutputCollector | None = None
        # llm-scaler v51 (F9): same leak-close as generate() — abort on any
        # exit path that is neither a clean finish nor an explicit abort.
        finished_cleanly = False
        abort_initiated = False
        try:
            q = await self.add_request(
        ...
        # is cancelled. So, we abort the request if we end up here.
            ...
            # llm-scaler v51 (F9): fire-and-forget abort (see generate()).
            if q is not None:
                abort_initiated = True
                try:
                    asyncio.get_running_loop().create_task(
        ...
        # Unexpected error in the generate() task (possibly recoverable).
            ...
            if q is not None:
                # llm-scaler v51 (F9): see generate() — abort on any
                # non-clean, non-aborted exit path.
                if not finished_cleanly and not abort_initiated:
                    abort_initiated = True
                    try:
                        asyncio.get_running_loop().create_task(
                        ...
                        if self.log_requests:
                            logger.info(
                                "Request %s aborted (F9: encode() exited "
                                "without finish or abort).", request_id)
                    except RuntimeError:
                        pass
                q.close()
