#!/bin/bash
# t3k_chain.sh — boot df7t3k lane (v1.2.5t3k = t3f + crash-4 R1 root fix
# v6: v52l scheduler geometric clamp/skip + core v52l EMPTY-SKIP + GMR
# v52g INPUT-SANITIZE).
# Validation targets:
#   - "v52l BOUNDARY-CLAMP"/"BOUNDARY-SKIP"/"BOUNDARY-SPAN-CORNER" fire
#     at 2048/4096/88064 zones; "v52l EMPTY-SKIP" tolerated on skips
#   - ZERO unreaped "v52d EMPTY-ROW" zombies; INPUT-SANITIZE lines
#     acceptable only on reaped/aborted requests
#   - ZERO worker deaths (no gather OOB; sanitize seatbelt holds)
#   - all P1-P8 finish normally or client-visible abort; engine SURVIVES
#     the full battery (streams crossing 2048 AND 4096)
LOG=/root/build/df7t3k_boot.out
{
...
nohup python3 /root/build/dt_loop8t07.py > /root/build/loop8t07_df7t3k.out 2>&1 &
