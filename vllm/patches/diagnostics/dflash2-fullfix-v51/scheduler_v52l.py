...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Diffusion models may not sample an autoregressive bonus token for a
        # denoising step; ordinary and speculative decoding sample one.
        ...
        # llm-scaler v48 (VLLM_XPU_DFLASH_TP1): shrink hash_block_size to
        # the gcd across all KV cache group block sizes so heterogeneous
        # pools (the TP1-replicated DFlash2 drafter installs a 1024-token
        # draft pool next to the 2048-token hybrid target pools) satisfy
        # HybridKVCacheCoordinator's "block_size % hash_block_size == 0"
        # assertion. No-op when every group shares one block size (all
        # stock lanes); 1024-token granularity for 2048-token blocks is a
        # supported combination ("the actual block size can be a multiple
        # of hash_block_size", kv_cache_coordinator.py).
        from math import gcd

        for _g in kv_cache_config.kv_cache_groups:
            hash_block_size = gcd(hash_block_size, _g.kv_cache_spec.block_size)
    ...
    def _mamba_block_aligned_split(
        ...
        if num_computed_tokens < max(request.num_prompt_tokens, request.num_tokens - 1):
            ...
            # llm-scaler v52g (crash-4 R1 ROOT FIX, 2026-09-06): decode /
            # spec-decode steps land here — the prefill gate above is False
            # once num_computed_tokens catches num_tokens (upstream's
            # "bypass normal decoding" NOTE assumes a decode step schedules
            # ONE token, which can never straddle a mamba block boundary
            # mid-step). Speculative decode breaks that assumption: a decode
            # step schedules (1 + k) rows (dflash k=7 -> 8), and when
            # num_computed_tokens % block_size falls in
            # [block_size - k, block_size - 1] the row range SPANS the
            # 2048-token mamba block boundary. The align-mode GDN state
            # subsystem (per-request slot window block_table[s .. s+k] plus
            # the fused conv+GDN spec path) does not support a mid-step
            # chunk crossing: every such step poisons the request's state
            # and the target emits ALL-NaN logits from then on (v52e), the
            # rejection kernel leaks its no-selection sentinel vocab_size
            # at pos 0, parse_output drops it, and the request becomes the
            # crash-4 zombie (permanently empty emission).
            #   t3e conviction battery (v1.2.5t3e, dt_loop8t07):
            #   - spanning crossings 2043/2043/2044 (+4089 for the request
            #     that survived its first, aligned crossing): 4/4 NaN
            #     zombies, PRE-COPY same-second, INPUT-OOB probe silent;
            #   - control crossing at computed=2048 EXACTLY (window
            #     2048..2055 inside one block): clean;
            #   - prefill chunks are block-aligned above and never NaN;
            #     nospec 1-row steps cannot span.
            # Restore the invariant the kernel subsystem requires: clamp a
            # decode-shaped step to the remaining room in the current mamba
            # block. The next step then starts block-aligned (the
            # empirically clean pattern). Gate on num_new_tokens <
            # block_size so aligned multi-block prefill chunks (whose
            # chunked-varlen kernel handles internal block boundaries) are
            # untouched, and on num_computed_tokens >= num_prompt_tokens so
            # prompt-phase chunks are untouched. Cost: at most k rows of
            # scheduling slack once per block_size tokens per request
            # (<= 7/2048 here). Downstream already supports per-request
            # variable draft counts: the spec-token attach trims to the
            # clamped num_new_tokens (num_scheduled_spec_tokens derives
            # FROM it), the gdn/mamba metadata builders are variable-width,
            # and the deferred accepted-count corrections are data-driven.
            #   - v52j (t3i) FALSIFIED: straddle->1-row degrade is fatal even
            #     engine-consistent (worker death, gather OOB); width-1/0-draft
            #     spec steps convicted fatal both sides (t3g worker-trim, t3i
            #     engine-consistent); plain-decode routing impossible (GMR
            #     batch-wide use_spec_decode gate swallows 0-draft entries).
            #   - v52l: lo = est - num_output_placeholders (self-correcting
            #     uncertainty bound, no prev tracking); SAFE(w) <=> no boundary
            #     in (lo, est+w); w_max = next_B(est-of-lo) - est; w>=2 clamp
            #     to w_max; exact corner (lo==est, w_max==1) -> minimal 2-row
            #     deliberate span (only surviving action; w=1 fatal, w=0
            #     deadlocks); straddle / corner-with-uncertainty -> skip tick
            #     (report collapses uncertainty; 0 flows into existing skip
            #     guard); sole-runnable-request fallback = 2-row span (empty
            #     steps assert on workers).
            #   - v52l: v52k minus the forced-span fallback (0-token
            #     dispatches are stock-legal: gpu_worker gates forward on
            #     total > 0; the core EMPTY-SKIP originally credited was
            #     convicted harmful in t3k/t3l and reverted in t3m;
            #     crossings execute only fully-reported).
            if (
                1 < num_new_tokens < self.cache_config.block_size
                and num_computed_tokens >= request.num_prompt_tokens
            ):
                block_size = self.cache_config.block_size
                est = num_computed_tokens
                # Uncertainty bound: est can be ahead of the worker's actual
                # position by the un-reported scheduled tokens, which the
                # async bookkeeping tracks in num_output_placeholders.
                # Self-correcting: it decays toward 0 as acceptance reports
                # land, so lo collapses to est exactly once nothing is in
                # flight.
                lo = max(
                    0, est - getattr(request, "num_output_placeholders", 0)
                )
                # First mamba block boundary strictly after lo.
                b_next = ((lo // block_size) + 1) * block_size
                # SAFE(w) <=> no block boundary B with lo < B < est + w
                # (i.e., the executed window is contained for every possible
                # actual start position in [lo, est]).
                w_max = b_next - est
                if w_max >= num_new_tokens:
                    # fully contained, worst case included: no clamp
                    pass
                elif w_max >= 2:
                    logger.warning(
                        "llm-scaler v52l BOUNDARY-CLAMP: %s est=%d lo=%d "
                        "w_max=%d num_new=%d blocksz=%d (clamped to end "
                        "exactly at the mamba block boundary; contained "
                        "for all possible actual starts)",
                        request.request_id,
                        est,
                        lo,
                        w_max,
                    ...
                    num_new_tokens = w_max
                elif w_max == 1:
                    if lo == est:
                        # Exact corner (est == boundary-1): only widths 0
                        # and 1 are contained; width-1 spec steps are
                        # convicted fatal (t3g/t3i) and width-0 cannot
                        # advance the request. Only surviving action: a
                        # minimal 2-row step spanning exactly one row into
                        # the next block (counts exact).
                        logger.warning(
                            "llm-scaler v52l BOUNDARY-SPAN-CORNER: %s "
                            "est=%d lo=%d w_max=%d num_new=%d blocksz=%d "
                            "(exact corner; minimal 2-row deliberate "
                            "span, 1 row past the boundary)",
                            request.request_id,
                            est,
                            lo,
                            w_max,
                        ...
                        # Corner zone with in-flight uncertainty: skip
                        # this tick; the pending acceptance report will
                        # move est and may dissolve the corner. Returning 0
                        # flows into the existing num_new_tokens <= 0 skip
                        # guard below.
                        logger.warning(
                            "llm-scaler v52l BOUNDARY-SKIP: %s est=%d "
                            "lo=%d w_max=%d num_new=%d blocksz=%d (corner "
                            "zone, in-flight uncertainty; skipped this "
                            "tick)",
                            request.request_id,
                            est,
                            lo,
                    ...
                    # Straddle: [lo, est] crosses a boundary; only widths 0
                    # and 1 are contained and width-1 is convicted fatal.
                    # Skip; next tick the report collapses the uncertainty.
                    logger.warning(
                        "llm-scaler v52l BOUNDARY-SKIP: %s est=%d lo=%d "
                        "w_max=%d num_new=%d blocksz=%d (straddle; "
                        "skipped this tick)",
                        request.request_id,
                        est,
                        lo,
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if self.need_mamba_block_aligned_split:
                ...
                # llm-scaler v52l (amended t3n): sole-request empty steps are
                # stock-LEGAL — gpu_worker.execute_model gates forward on
                # total_num_scheduled_tokens > 0 (no-op pass), so the boundary
                # skip above can always stand. (An earlier core-side
                # EMPTY-SKIP early-return was convicted harmful in t3k/t3l —
                # it starved the async walker's future.result() — and
                # reverted in t3m; the skip needs no core support.)

            # llm-scaler v52 (crash-4 fix, 2026-09-06 15:32): async
            # scheduling + spec-decode placeholder accounting can OVERSHOOT.
            # A zombie request whose engine outputs stop resolving (F9 gap:
            # request stays scheduled and live, num_computed_tokens keeps
            # advancing from engine-side state) accrues
            # num_output_placeholders += (1 + k) per step, and once the
            # books overshoot, num_new_tokens goes NEGATIVE (observed -7:
            # num_scheduled_tokens={req: -7}, total_num_scheduled_tokens=-7
            # in the crash dump). The == 0 guard below let the negative
            # flow through every clamp into SchedulerOutput, and
            # gpu_model_runner._prepare_inputs
            # assert total_num_scheduled_tokens > 0 killed BOTH TP workers
            # (every in-flight request -> HTTP 500). Treat <= 0 exactly
            # like == 0: skip this request for the step. The F8v2 guard in
            # async_scheduler.py force-finishes the zombie after a grace
            # period; `continue` (not break) keeps other requests
            # schedulable (same FCFS relaxation as the == 0 case).
            if num_new_tokens <= 0:
                ...
                #    (3) llm-scaler v52: negative placeholder overshoot
                #    (crash-4); skip until F8v2 reaps the zombie.
