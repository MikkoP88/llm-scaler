...
async def merge_async_iterators(
    *iterators: AsyncGenerator[T, None],
    ...
    if len(iterators) == 1:
        ...
        # llm-scaler v51 (F9): close the wrapped generator when this merge
        # wrapper is closed — previously the inner generator (async_llm
        # generate()) was only closed by GC, leaving engine requests alive
        # for a nondeterministic window when a serving consumer abandoned
        # the stream (crash-3 zombie leak; the multi-iterator path below
        # already acloses its children).
        try:
