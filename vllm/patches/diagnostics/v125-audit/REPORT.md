...
Harness: `serve_bench.sh / run_lane.sh / master125.sh / post125.sh` + `dt_probe2 / dt_ctxscan / dt_ctxscan2 / dt_conc` (pushed to `/root/build/`, archived in `.tmp-tq/`) and cross-audit `live_probe.py` (exact-token / natural-stop / barrier-sync probes, archived in this directory + `evidence/`). Long-context matrix prompts use VARIED 16-word fillers (KNOWN_ISSUES #13: repetitive fillers trigger instant-EOS ≥32k).

**Rev 2 (2026-09-07, after cross-audit review)**: D4 corrected (draft-KV inheritance), §3.1.1 mechanism re-attributed to FA2, SPLITS=64/G2 knob advice corrected to REJECT, exact-token 131,072/262,144-boundary + natural-stop evidence added (§2.2/§2.5), D1 mechanism explicitly marked unknown, harness limitations declared (§6). Pre-review snapshot: `evidence/REPORT-before-review.md`.
...
7. **Cross-audit correction round (applied to this revision)**: (a) D4 disproven — MTP draft KV inherits the target's `cache_config` (installed `llm_base_proposer.py:1188-1215`), it is *not* always-fp16; (b) fp8+nospec decode (q=1) routes through **FA2 with in-kernel fp8 dequant**, not the v51 SPLITS kernel (routing gate excludes q=1; forcing it via G2 was measured and REJECTED, −8.5…−61%); (c) `VLLM_FP8MQ_SPLITS=64` / `VLLM_TQ_MQ_SPLITS=64` advice removed — Phase I later measured both as regressions; (d) **exact-token probes** (server `/tokenize`, token-ID prompts): 131,072 ptok → 13.06 tok/s and 261,888 ptok + 256 gen = **262,144 total (exact maxlen boundary) served correctly** at 7.55 tok/s, finish=length, 0 errors; (e) natural-stop chat probes: 3/3 finish_reason=stop, 0 errors.
...
fp8 kernel routing as built (v51 design matrix, confirmed in gate code): **nospec decode (q=1) → FA2** (the v51 MQ gate excludes q=1; opt-in routing q=1 into the SPLITS kernel was Phase-I G2, measured REJECT −8.5…−61% vs FA2 varlen); MTP k4 verify (uniform q≤8) → v51 SPLITS, MTP drafter → v51 padded-uniform; DFlash k7 verify → v51 SPLITS, drafter → FA2 (non-causal excluded from the gate).
...
**Exact-token boundary probes** (cross-audit `live_probe.py`: server `/tokenize` → token-ID prompts, `usage['prompt_tokens']` validated == requested; prod lane mtp4+tq4nc @0.9/262144):

| Probe | prompt tokens | decode tok/s | TTFT | finish | errors |
|---|---|---|---|---|---|
| 128k exact | **131,072** | 13.06 | 86.1 s | length | 0 |
| maxlen exact | **261,888** + 256 gen = **262,144 total** | 7.55 | 228.6 s | length | 0 |

- The **exact 262,144 (maxlen) boundary is served correctly** — no off-by-one rejection, prompt_tokens verified, clean finish. Prior deep rows (136,191/242,761 ptok) were nwords-derived, i.e. 128k/227k-*class*, not exact-boundary; the exact probes bracket them consistently (13.06 vs 11.7 @128k-class, 7.55 vs 6.1 @262k-class, same lane & flags).
...
- Spec acceptance at depth (mtp4, 65k): per-position 0.730/0.714/0.143/0.016 → mean accepted ≈1.4 tok/step while paying 5 full-KV scans (target+drafts; draft KV dtype = target's, corrected D4). dflash7 keeps 0.808/0.692…@65k but pays 8 scans of a *separate* draft window (dtype = target's by default — `VLLM_DFLASH_DRAFT_KV_DTYPE` override exists) plus the slowest verify path (D2-adjacent for tq4).
...
### 2.5 Exact-token & natural-stop probes (cross-audit `live_probe.py`, prod lane mtp4+tq4nc @0.9/262144)

- **Chat natural-stop** (temperature 0, no `ignore_eos`, 3 prompts: arithmetic / JSON-constrained / stop-word): 3/3 `finish_reason=stop`, 0 errors, all with final usage and output SHAs recorded — no EOS-suppression pathology at temp 0 on this lane.
- **Barrier-synchronized short-ctx** (exact 2,048-token ID prompts): c1 71.7 tok/s decode; c4 (4 clients released simultaneously via barrier) aggregate 100.2 tok/s, 0 errors. Consistent with §2.1/§2.3 within noise; adds exact-prompt-token validation and per-event timestamps to the evidence set.
- Probe script archived at `vllm/patches/diagnostics/v125-audit/live_probe.py`; evidence JSONL in `vllm/patches/diagnostics/v125-audit/evidence/` (`v125_audit_fresh_short.jsonl` / `_deep.jsonl`).
...
### D4 — **CORRECTED (was wrong in first revision)**: draft KV dtype is *inherited from the target*, not always-fp16
- First revision claimed "MTP draft KV is always fp16 — no draft-KV dtype policy" (based on grep absence of `kv_cache_dtype` in the proposer). **Disproven by the installed code**: `llm_base_proposer.py:1188-1215` `_create_draft_vllm_config` builds the draft config from `base = self.vllm_config` and replaces only `kernel_config` (moe_backend) and `attention_config` — `cache_config` is inherited wholesale, so the draft pool's `kv_cache_dtype` **equals the target's** (tq4 target → tq4 draft; fp8 target → fp8 draft). Grep of the whole installed `v1/spec_decode/` tree confirms no MTP-path cache override.
- DFlash is the one drafter with an explicit policy (`dflash.py:25-29`, `:489`, `:514-531`): draft KV **defaults to the target's `--kv-cache-dtype`**, `VLLM_DFLASH_DRAFT_KV_DTYPE` overrides, `VLLM_DFLASH_TQ_DRAFT_KV=1` (default) enables TQ draft groups; `dflash2.py:175` only overrides `block_size`. The runner supports draft≠target TQ slot sizes (v21c policy, installed `gpu_model_runner.py:7279-7312` — `tq_slot` from `kv_cache_spec` is authoritative).
- Consequence: the "2× draft scan bytes" penalty story for mtp4-vs-nospec at depth is **withdrawn**; the measured spec-negative-at-depth result (D5) stands on acceptance collapse + (k+1) scans alone. Residual lever: an MTP-side override knob (mirror of `VLLM_DFLASH_DRAFT_KV_DTYPE`) would allow draft≠target *experiments* (e.g. fp8 draft under a tq4 target), but there is no measured deficit to fix — L3 demoted accordingly.
...
**Mechanism: UNKNOWN — the A/B convicts the axis, not a code path.** Facts on record: pool at 0.9 = 705-708k tokens (12.3 GiB) vs ~600k at 0.85; graphs captured 37/37 at 0.9 (clean); `VLLM_FP8MQ_SPLITS` env-fixed 32, kernel scratch grow-only cached, no pool-size dependence in `triton_fp8_mq.py`; hbs=1024 both. **Unverified hypotheses only** (none instrumented yet): spec-verify path taking a different route at larger pool (instrument the `_V51_FP8_MQ` gate hit-rate), draft-KV pool sizing squeeze at 0.9, or allocator-layout effect on the two-stage kernel. Needs a profiling session (step-time breakdown at 0.85 vs 0.9, 16k ctx, bs1 spec) before any of these can be stated as cause.
...
fp8 nospec decode (q=1) runs through **FA2 with in-kernel fp8 dequant** — not the v51 SPLITS two-stage kernel, whose uniform-q gate excludes q=1 (§1 routing note). Forcing q=1 into the SPLITS kernel was tried in Phase I (G2, `VLLM_XPU_FP8_MQ_Q1`): numerically sound but **REJECTED, −8.5…−61% vs FA2 varlen** — so FA2's in-kernel dequant is genuinely the right nospec path, and it beats the TQ dequant-to-gmem path by >2× at 227k. Direct evidence that the TQ decode kernel rewrite (L1) has ≥2× headroom at depth. Caveats: fp8 KV scale = 1.0 default (v51 certified accuracy posture), pool 705k tokens ⇒ max ~2.7 concurrent max-len requests (vs 4.9 for tq4), first-deep-request JIT (L7 covers).
...
| **L3** | **CLOSED (D4 corrected)**: draft KV dtype already inherits the target's — no bytes deficit to fix. Optional follow-up only: add an MTP-side `VLLM_MTP_DRAFT_KV_DTYPE` override (mirror of dflash's) *for experimentation* (e.g. fp8 draft under tq4 target); no measured deficit justifies it today | `llm_base_proposer.py` | None expected (was: halve draft scan bytes — premise withdrawn) | — | n/a (re-open only if P-1 profiling implicates draft-KV size) |
...
| L11 | (parked) prefix-cache 4096-token granularity (#10); MNBT 16384 rejected; KV splits already optimal (MQ 32, graph 256+tier — **SPLITS=64 measured REJECT for both TQ (−11/−10/−18%) and fp8 (regress-all), do not set**) | — | — | — | — |

**Ordering rationale:** L0 is free and validated; P-1 unblocks the user's unmodified flags; L1 has the largest kernel ceiling (measured ≥2.2× via the fp8 lane); L2 makes spec viable at depth (L3 closed — see D4 correction); L4 fixes concurrency cliffs; L6/L7 are latency hygiene; L10 is a small config-class fix.
...
Env knobs: keep defaults (TQ MQ splits 32; graph grid 256 w/ tier; `VLLM_XPU_FP8_MQ=1`). **Do NOT set `VLLM_FP8MQ_SPLITS=64` or `VLLM_TQ_MQ_SPLITS=64`** — both were measured REJECT later in Phase I (fp8: regress-all lanes; TQ: −11/−10/−18%), superseding the earlier "+34% @32k" single-point result. Likewise do not enable G2 (`VLLM_XPU_FP8_MQ_Q1`) — routing nospec q=1 decode into the SPLITS kernel loses 8.5-61% to FA2 varlen. Graceful `docker stop -t 30` before rm (KNOWN_ISSUES #03 — xe wedge).
...
- `live_probe.py` (cross-audit, archived in `vllm/patches/diagnostics/v125-audit/`) — exact-token HTTP probe: server `/tokenize` → token-ID prompts, barrier-synchronized clients, mandatory final `usage` (validated == requested prompt tokens), per-event timestamps, output SHAs; modes `chat` (natural stop) / `short` / `deep` (131,072 / 261,888) / `deepmulti`.
- Regression gates: battery SHA set, ctxscan ±10% of this report's table, deep decode ±10%, conc8 agg ±10%, 0 ERRS/STRIKES, chat natural-stop 3/3 `finish_reason=stop`, exact-262,144-boundary probe 0 errors.

**Harness limitations (declared, per cross-audit review):**
- **N=2 TTFT percentiles are not fairness evidence** — p50==p95 with two samples is just min==max; D8's "both prefilled together" claim needs the barrier+timestamp evidence class of `live_probe.py` (or N≥8) before it can quantify tail behavior. The D8 *asymmetry* itself (2-4× per-client decode gap) is measured on full 256-token streams, so it stands.
- **`ignore_eos: True` in all matrix decode tests** — throughput numbers are capacity-class measurements; they say nothing about natural stopping. Natural stop is now separately evidenced (§2.5, 3/3 chat stop), but only on 3 short prompts, one lane — not a coverage claim.
- nwords-based deep prompts are 128k/227k-*class* (136,191/242,761 ptok), not exact boundaries; exact-boundary coverage is §2.2's probe table (mtp4+tq4 lane only — other lanes' exact-boundary behavior untested).
- Un-tested / open items for a future round: fp8+spec deep at GMU 0.85 beyond 65k ctxscan (B/C deep cells exist; A-vs-C at exact 262,144 boundary not run), `deepmulti` (2 clients at 131k/262k) on any lane, natural-stop on non-prod lanes.
