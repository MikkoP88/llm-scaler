    ...
    def sample_tokens(
        ...
        if spec_config is not None:
            ...
            if use_gpu_toks:
                ...
                # llm-scaler v52n (crash-4 escalation-B root fix, t3n
                # lane): the RAW rejection-sampler row of a NaN-zombie
                # request carries the OOB bonus sentinel (argmax over
                # all-NaN target logits -> == vocab_size, observed
                # 248320) in slot 0; feeding it raw to the drafter
                # (propose below or prepare_next_token_ids_padded)
                # drives an out-of-range index into the drafter
                # embedding gather -> IndexKernelUtils.h:63 assert ->
                # worker death -> EngineDeadError (t3m_engine.log:
                # onset step == fatal step, PRE-COPY computed=4096
                # sched=8 at 23:42:22, worker died 23:42:31). Healthy
                # rows never contain >= vocab ids, and -1 rejection
                # tails are normal padded-kernel transport, so clamp
                # ONLY >=vocab entries on a per-step CLONE (two tiny
                # elementwise kernels, no device sync, no measurable
                # cost). The ORIGINAL tensor is untouched: state
                # updates, the async output copy and
                # RejectionSampler.parse_output still see the
                # sentinel, so the emission still resolves EMPTY and
                # the scheduler-side v52m strike-out aborts the
                # request client-visibly while the engine stays up.
                _v52n_vocab = self.model_config.get_vocab_size()
                sampled_token_ids = sampler_output.sampled_token_ids
                _v52n_row = sampled_token_ids.clone()
                _v52n_row.masked_fill_(_v52n_row >= _v52n_vocab, 0)
                sampled_token_ids = _v52n_row
        ...
        with record_function_or_nullcontext(
            ...
            # the empty-row attribution dump in get_output() (refs only,
            # per-step object). v52o: self.rejection_sampler EXISTS ONLY in
            # spec mode — stock never assigns it for nospec — and this
            # producer runs in EVERY mode (sample_tokens async branch), so
            # the bare attribute access killed both workers on the first
            # nospec request (t3n loopdiff nost3n leg, gpu_model_runner.py
            # sample_tokens AttributeError). Guard via getattr on self;
            # spec-mode tuple values are byte-identical, nospec now yields
            # (None, logits, None, None, None) which the v52e consumer's
            # try/except already tolerates.
            _v52e_rs = getattr(self, "rejection_sampler", None)
            async_output._v52e_refs = (
                spec_decode_metadata,
                logits,
                getattr(_v52e_rs, "_v52e_bonus", None),
                getattr(_v52e_rs, "_v52e_targ", None),
