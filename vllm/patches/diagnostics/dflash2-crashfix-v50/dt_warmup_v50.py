...
def main() -> None:
    ...
    # llm-scaler v50 (F4): two more shapes from the crash telemetry. The
    # R2/R3 crash cell (2026-09-05) died mid-LONG-decode: the first-use JIT
    # of the deeper-context TQ/propose specializations (~5.1 s stalls in the
    # crash logs) happened under user traffic, not at boot. p4 warms the
    # concurrent long-decode mixed batch; p5 warms chunked prefill (long
    # prompt) so the prefill-side kernels JIT before user traffic too.
