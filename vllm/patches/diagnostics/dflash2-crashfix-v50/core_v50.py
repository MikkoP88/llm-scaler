...
class EngineCore:
    ...
    def __init__(
        ...
        if vllm_config.cache_config.enable_prefix_caching or kv_connector is not None:
            ...
            # llm-scaler v50: remember the granularity this hasher closure
            # captured; the block below re-syncs it against the final pool
            # hash_block_size (VLLM_XPU_DFLASH_TP1 draft spec remap can
            # shrink the pool 2048->1024; starved cache_full_blocks ->
            # fatal assert block_pool.py:240).
            self._v50_hasher_hbs = hash_block_size

        # llm-scaler v50: re-sync the request block hasher to the FINAL pool
        # hash granularity (the scheduler + block pool were constructed above
        # from the post-remap group specs; the hasher closure captured the
        # resolve_kv_cache_block_sizes value, which can be coarser, e.g.
        # 2048 vs the pool's 1024 under VLLM_XPU_DFLASH_TP1's dflash2 v48
        # draft-spec remap). A coarser hasher starves cache_full_blocks of
        # hashes -> fatal assert (crash R2/R3, 2026-09-05). Only the starving
        # direction (pool < captured) is rebuilt; requests do not exist yet.
        _v50_final_hbs = getattr(
            getattr(
        ...
        _v50_captured_hbs = getattr(self, "_v50_hasher_hbs", None)
        if self.use_spec_decode:
            logger.info(
                "llm-scaler v50: hasher-resync probe: captured_hbs=%s "
                "pool_hbs=%s hasher_built=%s",
                _v50_captured_hbs,
                _v50_final_hbs,
                self.request_block_hasher is not None,
            )
        if (
            _v50_final_hbs is not None
            and _v50_captured_hbs is not None
            and _v50_final_hbs < _v50_captured_hbs
            ...
            self._v50_hasher_hbs = _v50_final_hbs
            logger.warning(
                "llm-scaler v50: request block hasher re-synced to final "
                "pool hash granularity %d -> %d (post-remap topology)",
                _v50_captured_hbs,
                _v50_final_hbs,
            )

        # llm-scaler v50 F6: fp8-class KV + speculative decoding is a
        # CONVICTED BROKEN lane on this XPU/B70 stack (v50 matrix
        # B5/B5b/B5d/B5e, 2026-09-05): the multi-row spec verify forward
        # on fp8 KV runs ~5 s/step (dflash k7 8-row: 0.3-0.9 tok/s and
        # engine-death wedge in the sample_tokens RPC on stock v1.2.1;
        # MTP k4 5-row: 0.2-0.5 tok/s) while the same pool serves
        # 22-34 tok/s nospec single-row. Refuse the combination at boot.
        if self.use_spec_decode:
            # fork uses cache_dtype (config/cache.py); upstream-style
            # builds use kv_cache_dtype — probe both.
            _v50_kvd = str(
                getattr(vllm_config.cache_config, "cache_dtype", None)
                or getattr(vllm_config.cache_config, "kv_cache_dtype", None)
                or ""
            )
            if _v50_kvd in ("fp8_e4m3", "fp8_e5m2", "fp8"):
                raise RuntimeError(
                    "llm-scaler v50 F6: speculative decoding with "
                    f"kv_cache_dtype={_v50_kvd} is refused: the multi-row "
                    "verify forward on fp8 KV is catastrophically slow on "
                    "this stack (0.2-0.9 tok/s, engine-death wedge risk; "
                    "convicted v50 B5/B5b/B5d/B5e). Use a TurboQuant KV "
                    "dtype for speculative decoding, or disable spec — "
                    "the fp8 nospec lane is certified at 34.4/30.7/22.4 "
                    "tok/s @2k/16k/65k."
                )
