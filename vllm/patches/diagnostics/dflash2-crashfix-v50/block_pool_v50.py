...
class BlockPool:
    ...
    def cache_full_blocks(
        ...
        # llm-scaler v50: hash-starvation clamp. The assert below assumed the
        # request block hasher always yields floor(all_tokens/hbs) hashes at
        # the pool's hash_block_size. Under VLLM_XPU_DFLASH_TP1 the draft
        # spec remap (dflash2 v48) shrinks the pool hash granularity
        # (2048->1024) AFTER the hasher closure captured the pre-remap value,
        # so a group can demand more full-block hashes than exist (fatal
        # assert, crash R2/R3 2026-09-05). Caching fewer blocks is always
        # safe — the prefix cache merely misses them; dying is not.
        _v50_max_nfb = (
            len(request.block_hashes) * self.hash_block_size
        ) // block_size
        if num_full_blocks > _v50_max_nfb:
            logger.error(
                "llm-scaler v50: hash-starvation clamp gid=%d bs=%d hbs=%d "
                "req=%s num_full_blocks %d -> %d (hashes=%d all=%d "
                "computed=%d ph=%d spec=%d)",
                kv_cache_group_id,
                block_size,
