...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def _greedy_sample_impl(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if hidden_states.shape[0] == batch_size * num_spec and num_spec > 1:
            ...
            # project_bias + the logits add allocate ~2 x [B, vocab] per
            # position per step. Reuse one grow-only bias scratch and add in
            # place so the eager drafting loop is allocation-free after the
            # first call — fresh per-step [B, vocab] temporaries churn the
            # XPU caching allocator and wedge the xe engines under PIECEWISE
            # XPU graphs when free memory is low.
            w2 = markov_head.markov_w2.weight
            bias_buf = getattr(self, "_markov_bias_buf", None)
            if (
                bias_buf is None
                or bias_buf.shape[0] < batch_size
                or bias_buf.shape[1] != w2.shape[0]
                or bias_buf.dtype != w2.dtype
                or bias_buf.device != w2.device
            ):
                if bias_buf is not None:
                    self._retired_markov_bufs = getattr(
                        self, "_retired_markov_bufs", []
                    )
                    self._retired_markov_bufs.append(bias_buf)
                capacity = (
                    max(batch_size, bias_buf.shape[0] * 2)
                    if bias_buf is not None and bias_buf.device == w2.device
                    else batch_size
                )
                bias_buf = torch.empty(
                    (capacity, w2.shape[0]),
                    dtype=w2.dtype,
                    device=w2.device,
                )
    ...
    def set_inputs_first_pass(
        ...
        # llm-scaler era-3 (DFlash2): a proposer may declare num_query_per_req
        ...
        num_query_per_req = (
            getattr(self, "num_query_per_req", 0) or self.num_speculative_tokens
        )
        ...
        # llm-scaler v49b (DFlash2 perf): CPU-side seq_lens UPPER BOUND for
        # the draft cam. TurboQuantMetadataBuilder.build copies
        # cam.seq_lens_cpu_upper_bound into TurboQuantMetadata.seq_lens_cpu;
        # left None (stock), every TQ _prefill_attention call — 5 draft
        # layers per propose — fell back to a synchronizing
        # seq_lens.tolist() D2H (turboquant_attn ~line 1024). Under
        # --async-scheduling the first drain per propose blocks the host
        # behind the overlapped target queue: measured propose h-d == 36.2
        # ms == tforward d exactly, TQTIME fwd 7.5 ms/call vs 0.45 on the
        # MTP lane (whose cam sets this field, llm_base_proposer ~1164).
        # Source: cad.seq_lens_cpu_upper_bound (the runner captures
        # optimistic_seq_lens_cpu into it BEFORE async-scheduling nulls
        # _seq_lens_cpu — "GPU tensors authoritative in async mode" — so it
        # is the only CPU copy that survives; sourcing _seq_lens_cpu made
        # the first v49b attempt inert). True draft seq_lens =
        # cad.seq_lens - rejected + num_query_per_req with rejected >= 0
        # known only on device, so this CPU tensor + num_query_per_req is
        # an upper bound. It only feeds host-side dispatch (the
        # q_len == seq_len first-chunk check; an upper bound steers to the
        # continuation path, always valid for drafts since context KV is
        # precomputed into the draft pool) and debug logs; all kernel
        # extents still come from the dynamic device seq_lens. Exact when
        # the batch carries no rejected tokens.
        _ub_src = getattr(cad, "seq_lens_cpu_upper_bound", None)
        if _ub_src is not None:
            _ub_src = _ub_src[: cad.num_reqs]
        if not getattr(self, "_v49b_logged", False):
            self._v49b_logged = True
            logger.info(
                "DFlash v49b: draft cam seq_lens_cpu_upper_bound attached=%s"
                " (kills the per-layer TQ seq_lens.tolist() D2H sync)",
                _ub_src is not None,
            )
        ...
        new_cad = CommonAttentionMetadata(
            ...
            seq_lens_cpu_upper_bound=(
                _ub_src + num_query_per_req if _ub_src is not None else None
            ),
