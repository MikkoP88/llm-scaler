...
# llm-scaler v23 (KNOWN_ISSUES #11): prefill-step peer barrier for the spec
# drafter. Resolved once at import; VLLM_SPEC_PREFILL_BARRIER=0 restores the
# v22 behavior (no barrier). See propose_draft_token_ids for the rationale.
...
class GPUModelRunner(
    ...
    def propose_draft_token_ids(
        ...
        # llm-scaler v23 (KNOWN_ISSUES #11): prefill-step peer barrier. With
        # TP>1 the drafter (MTP/EAGLE/dflash) issues its own TP collectives
        # immediately after the large prefill forward. If the ranks reach the
        # drafter skewed - one host still draining its prefill stream, the
        # peer already submitting head collectives - oneCCL's
        # non-preemptible spin-wait kernels can miss the rendezvous and burn
        # both engines forever (prefill has no competing queue work, so
        # unlike the deep-stream variant of this race there is no GuC
        # preempt reset: engines sit 100% spinning, EU stall-dominant, dmesg
        # clean, and the request stays Running until the client aborts).
        # Draining this rank's stream here bounds the peer-late window to
        # host glue on both ranks. Decode steps (max_num_seqs * (k+1)
        # tokens - 320 at 64 seqs / k=4) stay on the async fast path.
        if (
            _SPEC_PREFILL_PEER_BARRIER
            and num_scheduled_tokens >= 512
        ):
