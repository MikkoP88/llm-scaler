#!/bin/bash
# Reference serving command for qwen3.8-27b(-fp8) on llm-scaler-vllm-adv:v24
# (2x Intel Arc Pro B70, TP=2). Evolves patches/legacy/mtp-cudagraphs-eager-head-v22/serve.sh:
...
#   - SPEC_METHOD=dflash (default; needs DRAFTER_DIR) | mtp (target-internal
#     head, no drafter mount)
...
#   - v23: VLLM_SPEC_PREFILL_BARRIER (default 1, in-image) - prefill-step
#     barrier before the spec drafter. v23's one-sided drain did NOT fix
#     the #11 wedge (disproven live); v24 turns the same knob into a TRUE
#     peer rendezvous: stream drain + gloo cpu_group barrier before the
#     drafter on steps >= 512 tokens. VLLM_SPEC_PREFILL_BARRIER=0 restores
#     v22.
#
# Usage:
...
IMAGE="${IMAGE:-llm-scaler-vllm-adv:v24}"
NAME="${NAME:-lsv-test}"
PORT="${PORT:-8000}"
SPEC_K="${SPEC_K:-4}"
...
for _v in VLLM_XPU_TQ_SAFE_WARMUP VLLM_XPU_SPEC_SAFE_WARMUP \
          ...
          VLLM_XPU_MTP_EAGER_HEAD VLLM_SPEC_PREFILL_BARRIER; do
