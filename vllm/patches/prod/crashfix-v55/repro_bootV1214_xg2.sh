...
# v59 §1: + patch_stall_v59.py in boot chain (tool-call stream stall fix —
# Responses SSE dies on Qwen3XML name=None first tool delta; STALFIX A/B/C)
# + VLLM_GUIDED_DECODING_BACKEND=xgrammar env (v0-era var, no-op in this
# fork; v1 structured-output auto-selects xgrammar which is present).
set -eu
...
docker run -td --privileged --name lsv-test \
    ...
    -w /llm-scaler/vllm \
    --entrypoint /bin/bash \
    llm-scaler-exp:v1.2.14

echo "BOOT_$MODE container launched (image v1.2.14) $(date +%H:%M:%S)"
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_v55_3.py | tee /root/build/lce1/v55_3_apply_bootV1212.log
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_stall_v59.py | tee /root/build/lce1/stall59_apply_bootV1212.log
...
echo "BOOT_$MODE serve started (v1.2.14 baked crash-free config) $(date +%H:%M:%S)"
...
for i in $(seq 1 60); do
  ...
  if [ "$code" = "200" ]; then
    ...
    echo "PROD_STANDING_V1213_STALFIX_XG7 $(date -u +%H:%M:%S)"
    exit 0
  fi
