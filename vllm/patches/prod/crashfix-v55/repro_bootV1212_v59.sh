...
docker run -td --privileged --name lsv-test \
    ...
    -w /llm-scaler/vllm \
    --entrypoint /bin/bash \
    llm-scaler-exp:v1.2.13

echo "BOOT_$MODE container launched (image v1.2.13) $(date +%H:%M:%S)"
...
echo "BOOT_$MODE serve started (v1.2.13 baked crash-free config) $(date +%H:%M:%S)"
