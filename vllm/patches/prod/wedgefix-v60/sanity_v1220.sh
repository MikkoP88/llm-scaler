...
# + v63/v64/v66 baked checks + v1220 stamps + admission probe (the v66
...
R=/root/build/_v1220_sanity.txt
: > $R
B=http://127.0.0.1:8000
echo "== SANITY READY-v1220 ==" >> $R
curl -s -m 120 $B/v1/chat/completions -H 'Content-Type: application/json' \
  -d '{"model":"qwen3.8-27b-fp8","max_tokens":2000,"messages":[{"role":"user","content":"Reply with exactly this token and nothing else: READY-v1220"}]}' \
...
echo "== V1220 STAMPS ==" >> $R
docker exec lsv-test sh -c 'ls -la /root/.llm_scaler_exp_v1220_baked /root/.v1220_jit_warmed /root/.v64_jit_warmed 2>&1' >> $R 2>&1
