#!/bin/bash
# §24 Q COH lane: repro_bootBS64_E4M3_G85NS_MB4K + patch_v24pc copy-on-hit.
# Identical to repro_bootBS64_E4M3_G85NS_MB4K.sh (e4m3 bs64 gmu0.85 mnbt4096
# no-async MTP4 PC-ON) with patch_v24pc applied after patch_v58_p1 and
# VLLM_COH=1 VLLM_COH_MIN_BLOCKS=16 in the container env. Patcher failure
# aborts the boot (set -e + grep -c verify) -> BOOT_FAIL visible in runner.
set -eu
MODE="${1:?usage: repro_bootBS64_E4M3_G85NS_MB4K_COH.sh COH}"
...
docker run -td --privileged --name lsv-test \
    ...
    -e VLLM_COH=1 -e VLLM_COH_MIN_BLOCKS=16 \
...
docker cp /root/build/patch_v24pc.py lsv-test:/root/patch_v24pc.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_v24pc.py | tee /root/build/lce1/v24pc_apply_bootCOH.log
docker exec lsv-test grep -c "llm-scaler v24pc" /opt/venv/lib/python3.12/site-packages/vllm/v1/worker/gpu_model_runner.py
...
echo "BOOT_$MODE serve started (e4m3 bs64 PC-ON + v24pc copy-on-hit) $(date +%H:%M:%S)"
