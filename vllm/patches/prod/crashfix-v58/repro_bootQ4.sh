#!/bin/bash
# repro_bootQ4.sh — Boot Q4: arstage v2 CENSUS boot. EXACT Boot Q lane
# (e5m2@262144 + mtp4, pidfd, v1.2.10, oneCCL reduce path) + patch_f15b
# v3 (transition watchdog, ring 4096) + patch_arstage v2 (persistent
# small-AR staging, cap 262144, with per-class staged/direct census
# keyed (numel,dtype,contig,stream), non-contig copy-staged,
# VIA_ALLGATHER guard, ARSTAGE_CENSUS line in every f15b dump).
# Q3 showed the wedge STILL hitting the 15360/5120 draft-propose class
# under the cap with the identical reset signature — Q4's dump answers
# whether those victims were STAGED (=> F12-as-scoped falsified) or a
# passthrough (=> coverage hole now closed by v2's copy-staging).
# Clean >=2x boot Q TTF (>=61 min under the full armature) graduates L.
set -eu
MODE="${1:?usage: repro_bootQ4.sh Q4}"
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ4.log
docker exec lsv-test /opt/venv/bin/python3 /root/patch_arstage.py | tee /root/build/lce1/arstage_apply_bootQ4.log
