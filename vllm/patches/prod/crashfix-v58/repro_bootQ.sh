#!/bin/bash
# repro_bootQ.sh — Boot Q: F15b instrumented diag boot. EXACT Boot M
# lane (e5m2@262144 + mtp4, pidfd, v1.2.10) + patch_f15b spec-step
# stall watchdog (completion-event ring + early dump at 45 s, BEFORE
# the 300 s RPC timeout / 600 s fences destroy live state). guc
# verbose on. Crash EXPECTED (class TTF 31 s-50 min) — the dump names
# the resident region / AR divergence.
set -eu
MODE="${1:?usage: repro_bootQ.sh Q}"
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ.log
