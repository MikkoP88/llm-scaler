#!/bin/bash
# repro_bootQ5.sh — Boot Q5: Fix M discriminator = AR single-stream
# FUNNEL. EXACT Boot Q lane (e5m2@262144 + mtp4, pidfd, v1.2.10,
# oneCCL reduce path — NO via-allgather, NO arstage) + patch_f15b v3
# (transition watchdog, ring 4096) + patch_arfunnel (every
# _all_reduce_impl executes on ONE dedicated per-device stream,
# event-ordered vs the caller stream both directions).
# Q4 falsified F12-as-scoped (victims 100% staged on persistent VAs)
# and convicted cross-stream collective ordering desync (>= 4 stable
# AR-issuing streams). Funnel restores per-connection FIFO device
# order = host order on BOTH ranks -> desync impossible.
# Graduation: >= 61 min clean under the full armature (= 2x boot Q
# TTF), then numerics battery + perf parity.
set -eu
MODE="${1:?usage: repro_bootQ5.sh Q5}"
...
docker cp /root/build/patch_arfunnel.py lsv-test:/root/patch_arfunnel.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ5.log
docker exec lsv-test /opt/venv/bin/python3 /root/patch_arfunnel.py | tee /root/build/lce1/arfunnel_apply_bootQ5.log
docker exec lsv-test grep -c "llm-scaler f15b" /opt/venv/lib/python3.12/site-packages/vllm/v1/worker/gpu_model_runner.py
docker exec lsv-test grep -c "llm-scaler arfunnel" /opt/venv/lib/python3.12/site-packages/vllm/distributed/device_communicators/xpu_communicator.py
