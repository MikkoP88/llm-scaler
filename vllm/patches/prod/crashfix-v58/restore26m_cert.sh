...
echo "[CERT] f8ref"
timeout 360 python3 -u f8ref.py RESTORE26M 2>&1 | tee lce1/f8ref_RESTORE26M.out | grep -o 'hashes=.*' || true
grep -q 'distinct=OK' lce1/f8ref_RESTORE26M.out && echo F8REF_DETERMINISTIC || echo F8REF_NONDET
grep -q 'cb8c3851b897' lce1/f8ref_RESTORE26M.out && echo F8REF_EXACT || echo F8REF_NOT_EXACT
echo "[CERT] q17 spot"
bash q17_perf.sh > lce1/q17_RESTORE26M.out 2>&1
tail -4 lce1/q17_RESTORE26M.out
