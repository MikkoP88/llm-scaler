#!/bin/bash
# §24 R probe lane: repro_bootBS64_E4M3_G85NS_MB4K + patch_v24r table dump.
# Identical to repro_bootBS64_E4M3_G85NS_MB4K.sh (e4m3 bs64 gmu0.85 mnbt4096
# no-async MTP4 PC-ON) with patch_v24r (read-only diagnostic) applied after
# patch_v58_p1 and VLLM_V24R=1 in the container env. Patcher failure aborts
# the boot (set -e + grep -c verify) -> BOOT_FAIL visible in runner.
set -eu
MODE="${1:?usage: repro_bootBS64_E4M3_G85NS_MB4K_V24R.sh V24R}"
...
docker cp /root/build/patch_v24r.py lsv-test:/root/patch_v24r.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_v24r.py | tee /root/build/lce1/v24r_apply_bootV24R.log
...
echo "BOOT_$MODE serve started (e4m3 bs64 PC-ON + v24r table dump) $(date +%H:%M:%S)"
