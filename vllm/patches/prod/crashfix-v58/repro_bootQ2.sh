#!/bin/bash
# repro_bootQ2.sh — Boot Q2: VIA_ALLGATHER discriminator. EXACT Boot Q
# lane (e5m2@262144 + mtp4, pidfd, v1.2.10) + patch_f15b v2 (fixed
# transition watchdog) + VLLM_XPU_ALLREDUCE_VIA_ALLGATHER=1 — swaps the
# oneCCL reduce kernel for allgather+local-add on every eager AR call
# (crash freeze point was an eager propose-phase collective). Clean >=2x
# boot Q TTF (>=61 min) convicts the oneCCL reduce kernel; a crash fires
# the fixed watchdog's in-process ring dump at 45 s.
set -eu
MODE="${1:?usage: repro_bootQ2.sh Q2}"
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ2.log
