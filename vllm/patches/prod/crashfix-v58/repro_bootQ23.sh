...
set -eu
MODE="${1:?usage: repro_bootQ23.sh Q23}"
...
nohup bash /root/build/mem_growth_logger.sh /root/build/lce1/bootQ23_memgrowth.log > /dev/null 2>&1 < /dev/null &

docker cp /root/build/patch_f15b.py lsv-test:/root/patch_f15b.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ23.log
docker cp /root/build/patch_arstage.py lsv-test:/root/patch_arstage.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_arstage.py | tee /root/build/lce1/arstage_apply_bootQ23.log
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_v58_p1.py | tee /root/build/lce1/v58p1_apply_bootQ23.log
...
echo "BOOT_$MODE serve started (stock NEO 26.14 + IMMEDIATE_CMDLISTS=1) $(date +%H:%M:%S)"
