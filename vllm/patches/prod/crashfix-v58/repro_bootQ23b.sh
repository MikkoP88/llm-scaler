#!/bin/bash
# Q23 = §21 lever: L0 immediate-command-list flip, ARM 1 (=1), stock
# NEO 26.14 image-native (NO deb overlay — single variable = env).
# §14-class = GSD-12919 platform race (§20); NEO lever closed by
# degradation (§21); this is the last container-local lever.
# Discriminator: solo/ctx2k vs baseline 50.4 / 407-414. If ==band ->
# knob inert (default already 1). If !=band -> default was 0.
set -eu
MODE="${1:?usage: repro_bootQ23bb.sh Q23}"
...
nohup bash /root/build/mem_growth_logger.sh /root/build/lce1/bootQ23b_memgrowth.log > /dev/null 2>&1 < /dev/null &

docker cp /root/build/patch_f15b.py lsv-test:/root/patch_f15b.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_f15b.py | tee /root/build/lce1/f15b_apply_bootQ23b.log
docker cp /root/build/patch_arstage.py lsv-test:/root/patch_arstage.py
docker exec lsv-test /opt/venv/bin/python3 /root/patch_arstage.py | tee /root/build/lce1/arstage_apply_bootQ23b.log
...
docker exec lsv-test /opt/venv/bin/python3 /root/patch_v58_p1.py | tee /root/build/lce1/v58p1_apply_bootQ23b.log
...
echo "BOOT_$MODE serve started (stock NEO 26.14 + IMMEDIATE_CMDLISTS=0) $(date +%H:%M:%S)"
