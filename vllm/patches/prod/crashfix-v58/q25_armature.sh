#!/bin/bash
# q22_armature.sh — Q25a (NEO 26.27) battery certification: §20 F.
...
nohup bash repro_sustain.sh 18 > lce1/sustain_Q25.out 2>&1 < /dev/null &
echo "sustain(18) launched $(date +%H:%M:%S)"
sleep 90
nohup bash p1_coh_watch.sh 200 > lce1/p1_coh_Q25.out 2>&1 < /dev/null &
echo "coh_watch launched $(date +%H:%M:%S)"
sleep 1740
echo "=== mid-load boundary sweep 1 (~30 min) ($(date +%H:%M:%S))"
python3 -u p1_len5probe.py 2>&1 | tee lce1/p1_len5probe_Q25_load1.out
sleep 3600
echo "=== mid-load boundary sweep 2 (~90 min) ($(date +%H:%M:%S))"
python3 -u p1_len5probe.py 2>&1 | tee lce1/p1_len5probe_Q25_load2.out
wait
