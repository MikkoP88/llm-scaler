...
class FlashAttentionBackend(AttentionBackend):
    ...
    def get_fp8_dtype_for_flashattn(kv_cache_dtype: str) -> torch.dtype:
        if kv_cache_dtype in ("fp8", "fp8_e4m3"):
            ...
            # llm-scaler v51: e5m2 KV is supported end-to-end on XPU — the
            # v51 Triton fp8 MQ kernel carries the IS_E5M2 bitcast branch
            # and the cache-reshard paths (below) already map it. This
            # helper was the last refusal site (raised during graph-capture
            # warmup, killing the boot).
        ...
        if not attn_metadata.use_cascade:
            ...
            if self.dcp_world_size > 1:
                ...
                if (
                    ...
                    # opt-out.
                    # llm-scaler v38: fp8 decode must not use the ESIMD kernel
                ...
                # Triton flash-decode kernel (vllm/v1/attention/ops/
                # triton_fp8_mq.py). Root fix for the fp8+spec slowness:
                # v50 B5b convicted tforward d=4976 ms with the v33 route
                # ACTIVE — the general-purpose unified_attention 3D-segment
                # kernel IS the slow path, not a missing route. This kernel
                # gives the shape real parallelism: grid (B, Hq, SPLITS),
                # one KV pass scores all q rows (per-row causal limits),
                # fp32 deterministic fixed-order combine. Covers MTP
                # verify (q=k+1 in 2..5), dflash verify (q=8), and the
                # draft-side multi-row forwards that share the uniform
                # shape. Rollback: VLLM_XPU_FP8_MQ=0 -> v33 route below.
                if (
                    _V51_FP8_MQ
                    ...
                    try:
                        from vllm.v1.attention.ops.triton_fp8_mq import (
                            triton_fp8_mq_attention as _v51_fp8_mq_fn,
                        )
                    except ImportError:
                        _v51_fp8_mq_fn = None
                    if _v51_fp8_mq_fn is not None:
                        _v125_p1_route("v51_uniform", num_actual_tokens,
                ...
                # llm-scaler v53 (v125-audit L4, 2026-09-07): RAGGED fp8
                # multi-row verify padded to the uniform shape so the v51
                # SPLITS kernel covers it too (D2: mixed finished/running
                # or chunked-tail batches previously fell to v33 MQ3D or
                # C++ FA2 branch1 — no SPLITS, pathological at depth).
                # Padding is per-request FRONT padding: each request's q_i
                # real rows are placed at slots [q-q_i, q) of its q-row
                # window, so kernel row j maps to position seq_len-q+j
                # EXACTLY as in a uniform batch — the per-row causal math
                # stays bit-exact for every real row; pad rows produce
                # discarded garbage (never read back). Gate: only shapes
                # the v33 block below would otherwise take (fp8, paged,
                # causal, 1<q<=8, no window/softcap/descale), only when
                # padding waste <= 50% (num_actual*2 >= q*B). Rollback:
                # VLLM_V53_L4=0 -> v33 route unchanged.
                if (
                    _V53_L4
                    and _V51_FP8_MQ
                    and isinstance(self.kv_cache_dtype, str)
                    ...
                    and q_descale is None
                    and self.head_size <= 256
                    and num_actual_tokens * 2
                    >= max_seqlen_q * (cu_seqlens_q.shape[0] - 1)
                ):
                    try:
                        from vllm.v1.attention.ops.triton_fp8_mq import (
                            triton_fp8_mq_attention as _v53_l4_fn,
                        )
                    except ImportError:
                        _v53_l4_fn = None
                    if _v53_l4_fn is not None:
                        _v125_p1_route("v53_l4_ragged_pad", num_actual_tokens,
                                       max_seqlen_q, cu_seqlens_q)
                        try:
                            _B = cu_seqlens_q.shape[0] - 1
                            _q = int(max_seqlen_q)
                            _n = num_actual_tokens
                            _starts = cu_seqlens_q[:-1].long()        # [B]
                            _lens = cu_seqlens_q[1:].long() - _starts  # [B]
                            _j = torch.arange(
                                _q, device=query.device).unsqueeze(0)  # [1,q]
                            _pad_left = (_q - _lens).unsqueeze(1)      # [B,1]
                            _valid = _j >= _pad_left                   # [B,q]
                            _src = _starts.unsqueeze(1) + (_j - _pad_left)
                            _src = _src.clamp(min=0, max=_n)
                            _src = torch.where(_valid, _src, _n)       # pad row
                            # padded query: real rows copied at natural
                            # offsets + one zero pad row at index _n,
                            # then gathered to [B*q, Hq, D].
                            _padded_q = query.new_zeros(_q * _B + 1,
                                                        query.shape[1],
                                                        query.shape[2])
                            _padded_q[:_n] = query[:_n]
                            _padded_q = _padded_q[_src.view(-1)]
                            _padded_out = torch.empty_like(_padded_q)
                            _v53_l4_fn(
                                self,
                                _padded_q,
                                key_cache,
                            ...
                            # scatter real rows back: request i's row j'
                            # (global row r) sits at slot i*q + (q-lens_i) + j'
                            _r = torch.arange(_n, device=query.device)
                            _i = torch.searchsorted(
                                cu_seqlens_q[1:].long(), _r, right=True)
                            _slot = (_i * _q + (_q - _lens[_i])
                                     + (_r - _starts[_i]))
                            output[:_n].copy_(_padded_out[_slot])
                            return output
                        except Exception:
                            # Any shape surprise -> fall through to v33
                            # (behavior identical to v1.2.5 for this batch).
                            _v125_p1_route("v53_l4_fallback", num_actual_tokens,
                                           max_seqlen_q, cu_seqlens_q)

                # llm-scaler v33: paged multi-query (MTP verify) on fp8 KV
                ...
                if (
                    ...
                    _v125_p1_route("v33_mq3d", num_actual_tokens,
                                   max_seqlen_q, cu_seqlens_q)
                    _v33_mq3d_varlen(
                        self,
                        query[:num_actual_tokens],
...
# llm-scaler v51: dedicated Triton fp8 multi-query flash-decode route
# (vllm/v1/attention/ops/triton_fp8_mq.py) for UNIFORM multi-row fp8
# verify. Takes precedence over the v33 unified_attention route above;
# v33 remains the fallback for non-uniform shapes and VLLM_XPU_FP8_MQ=0.
...
# llm-scaler v53 (v125-audit L4, 2026-09-07): pad RAGGED fp8 multi-row
# verify batches (mixed finished/running, chunked tails — anything where
# num_actual_tokens != max_seqlen_q * B) into the uniform shape so the
# v51 SPLITS kernel covers them instead of falling to v33 MQ3D / C++ FA2
# branch1 (REPORT D2: no SPLITS under concurrency at depth). Default ON
# (convicted defect, bit-exact per-row causal math via FRONT padding);
# rollback VLLM_V53_L4=0 restores the v1.2.5 routing exactly.
_V53_L4 = os.environ.get("VLLM_V53_L4", "1") != "0"

# llm-scaler v53 (v125-audit P-1, 2026-09-07): env-gated fp8 route-mix
# counters for the GMU x spec x fp8 profiling session (REPORT D1: the
# 0.9-vs-0.85 collapse convicts the axis but no code path; first
# instrumented hypothesis is "verify takes a different route at the
# larger pool"). Counters fire ONLY when VLLM_V125_P1=1 — zero cost
# otherwise (single env-bool check per routed call). Summary line every
# 2000 routed calls: per-route counts + the (B, q, uniform) shape mix.
_V125_P1 = os.environ.get("VLLM_V125_P1", "0") == "1"
_V125_P1_COUNTERS: dict = {}
_V125_P1_PERIOD = 2000


def _v125_p1_route(name, num_actual_tokens, max_seqlen_q, cu_seqlens_q):
    if not _V125_P1:
        return
    ent = _V125_P1_COUNTERS.setdefault(name, [0, 0])  # [calls, tokens]
    ent[0] += 1
    ent[1] += int(num_actual_tokens)
    total = sum(
        v[0] for k, v in _V125_P1_COUNTERS.items()
        if not k.startswith("_"))
    if total % _V125_P1_PERIOD == 0:
        try:
            b = int(cu_seqlens_q.shape[0]) - 1
            uni = num_actual_tokens == max_seqlen_q * b
        except Exception:
            b, uni = -1, False
        msg = " ".join(
            f"{k}={v[0]}({v[1]}tok)" for k, v in _V125_P1_COUNTERS.items()
            if not k.startswith("_"))
        logger.info(
            "V125_P1 routes: %s [last B=%d q=%s uniform=%s]",
            msg, b, max_seqlen_q, uni)
