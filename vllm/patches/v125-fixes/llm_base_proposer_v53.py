...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        # llm-scaler v53 (v125-audit L2, 2026-09-07): context-adaptive
        # emission width. Measured (v125 REPORT D5): per-position acceptance
        # collapses with depth (0.73/0.71/0.14/0.02 @65k for mtp4) while
        # every step pays (k+1) full-KV verify scans plus (k-1) eager draft
        # forwards — fixed-k speculation is NET-NEGATIVE at deep context
        # (mtp4 11.7 tok/s vs nospec 17.7 @128k). VLLM_SPEC_CTX_K
        # "<ctx_threshold>:<k>" reduces the emission width to k (floor 1;
        # never 0 — the v52m zero-emission strike-out treats sustained
        # empty emissions as a zombie signature) once EVERY request in the
        # decode batch has context >= threshold. Batch-minimum policy over
        # the CPU-side seq-len shadow: no device sync, no per-request host
        # round-trips; a mixed shallow/deep batch keeps full k. Default ""
        # = inert (identical to v1.2.5). For sequential drafters (mtp) the
        # draft loop is also cut to k-1 iterations — at k=1 the drafter
        # runs a single forward and verify sees 2 rows. Output tokens at
        # temp 0 are unchanged (greedy target accepts are prefix-exact),
        # so battery SHAs must stay bit-exact (L2 pass gate).
        self._v53_ctxk: tuple[int, int] | None = None
        _ctxk = os.environ.get("VLLM_SPEC_CTX_K", "")
        if _ctxk:
            try:
                _th, _k = _ctxk.split(":")
                _th, _k = int(_th), int(_k)
            except ValueError as e:
                raise ValueError(
                    f"VLLM_SPEC_CTX_K={_ctxk!r} invalid; want "
                    f"'<ctx_threshold>:<k>', e.g. '65536:1'"
                ) from e
            if _th <= 0 or not 1 <= _k <= self.num_speculative_tokens:
                raise ValueError(
                    f"VLLM_SPEC_CTX_K={_ctxk!r} invalid; want ctx_threshold > 0 "
                    f"and 1 <= k <= {self.num_speculative_tokens}"
                )
            self._v53_ctxk = (_th, _k)
            # llm-scaler v53 t2: CONVICTED UNSOUND under async scheduling
            # (Lane A, 2026-09-07 14:02-14:07): the first k=1 narrowing
            # during a prefill-overlap step wedged both TP workers
            # (5-min silent spin -> RPC 'sample_tokens' timeout ->
            # EngineDeadError). Root cause: AsyncScheduler plans the
            # spec width STATICALLY via _spec_token_placeholders
            # (v42c DFLASH2_EMIT_K contract: "the placeholder list
            # width IS the scheduled spec width ... everything
            # downstream derives from the scheduled width"); a
            # proposer-side dynamic narrowing desyncs placeholder /
            # num_output_tokens accounting under whole-step graphs.
            # Correct design (L2b, follow-up): decide the width in the
            # SCHEDULER (exact CPU ctx lives there), plumb it through
            # SchedulerOutput -> runner -> spec worker, and capture
            # graphs for both widths. Until then narrowing is gated
            # behind VLLM_V53_L2_UNSTABLE=1 (explicit
            # reproduce-the-wedge escape hatch); with the gate closed
            # the knob only ARMS + logs the would-fire decision
            # (observability, zero behavior change).
            self._v53_unstable = (
                os.environ.get("VLLM_V53_L2_UNSTABLE", "0") == "1"
            )
            logger.info(
                "v53 L2 ctx-adaptive k armed: all-ctx >= %d -> k=%d (full"
                " %d); narrowing %s (VLLM_V53_L2_UNSTABLE gate; async-"
                "sched wedge convicted in Lane A — see v125-fixes PLAN)",
                _th,
                _k,
                self.num_speculative_tokens,
                "ENABLED (unstable)" if self._v53_unstable else "DISABLED",
            )
        self._v53_ctxk_fires = 0
    ...
    def _v53_k_eff(self, common_attn_metadata: CommonAttentionMetadata) -> int:
        """llm-scaler v53 (v125-audit L2): adaptive emission width.

        Returns num_speculative_tokens unless VLLM_SPEC_CTX_K is armed AND
        every request in this decode batch has context length >= the knob
        threshold (batch minimum). Floor is 1: emissions are never empty
        (v52m zero-emission strike-out semantics). Any doubt (empty
        batch, unreadable lens) -> full width.

        Context-length source cascade (t2 fix — the t1 read of the
        deprecated _seq_lens_cpu shadow returned None on EVERY call:
        nothing in the spec path populates that lazy cache, so L2 armed
        but never fired; Lane A deep decode was bit-identical to v1.2.5
        and 0 'fired' lines proved it):
          1. seq_lens_cpu_upper_bound — resident CPU tensor, zero sync.
             Semantics (backend.py): optimistic for async-spec rows
             (assumes every draft accepted) => OVER-estimates real ctx
             by at most the spec width (<= 7 tok) — irrelevant against
             a >= 65536 threshold. Worst case of the one-sided error:
             k is cut a few tokens of depth early / late.
          2. _seq_lens_cpu shadow if populated.
          3. One explicit seq_lens.to('cpu') sync — ONLY reached when
             armed (knob unset in prod = zero cost here) and both CPU
             sources absent; <= ~0.1 ms against a ~100 ms deep-ctx
             step.
        The fire decision uses batch-min over the first n rows.
        """
        if self._v53_ctxk is None:
            return self.num_speculative_tokens
        th, k = self._v53_ctxk
        n = common_attn_metadata.batch_size()
        if n == 0:
            return self.num_speculative_tokens
        seq_cpu = getattr(
        ...
        src = "ub"
        if seq_cpu is None:
            seq_cpu = getattr(common_attn_metadata, "_seq_lens_cpu", None)
            src = "shadow"
        if seq_cpu is None:
            seq_cpu = common_attn_metadata.seq_lens.to("cpu")
            src = "sync"
        try:
            if len(seq_cpu) < n:
                return self.num_speculative_tokens
            mn = seq_cpu[:n].min()
            min_ctx = int(mn.item()) if hasattr(mn, "item") else int(mn)
        except Exception:
            return self.num_speculative_tokens
        if min_ctx >= th:
            self._v53_ctxk_fires += 1
            if self._v53_ctxk_fires == 1 or self._v53_ctxk_fires % 1000 == 0:
                logger.info(
                    "v53 L2 ctx-adaptive k fired (#%d, src=%s): min_ctx=%d"
                    " >= %d -> k=%d%s",
                    self._v53_ctxk_fires,
                    src,
                    min_ctx,
                    th,
                    k,
                    ""
                    if self._v53_unstable
                    else " [UNSTABLE gate closed: keeping full width]",
                )
            if self._v53_unstable:
                return k
            return self.num_speculative_tokens
    ...
    def _propose_impl(
        ...
        if self.num_speculative_tokens == 1 or self.parallel_drafting:
            ...
            # llm-scaler v53 L2 (v125-audit): parallel drafters (dflash)
            # compute all N candidates in one pass, so adaptive-k only
            # truncates EMISSION here. List form == the per-request-width
            # contract (same as _truncate_by_confidence / dflash2 v42
            # EMIT_K) — verify runs k+1 rows for these requests.
            _k_par = self._v53_k_eff(common_attn_metadata)
            if _k_par < self.num_speculative_tokens:
                return (
        ...
        # llm-scaler v53 L2 (v125-audit): sequential drafters (mtp) pay
        # (k-1) eager draft forwards per step (D6: 52-71% host samples in
        # the draft chain) — cut the loop to the adaptive width.
        _k_eff = self._v53_k_eff(common_attn_metadata)
        ...
        # llm-scaler v53 L2: loop bound is the adaptive width (full N when
        # the knob is inert or the batch is not uniformly deep).
        for token_index in range(_k_eff - 1):
            # Update the inputs.
        ...
        # [batch_size, k_eff]  (llm-scaler v53 L2: may be narrower than
        # num_speculative_tokens; list form signals per-request width to
        # the scheduler/verify paths — same contract as dflash2 v42.)
        draft_token_ids = torch.stack(draft_token_ids_list, dim=1)
        if _k_eff < self.num_speculative_tokens:
