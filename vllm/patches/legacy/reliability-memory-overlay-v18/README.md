# llm-scaler-vllm-adv:v18 — reliability + memory overlay (qwen3.8-27b, 2x Arc Pro B70)

`FROM llm-scaler-vllm-adv:v17`. Single-file delta (gpu_worker.py): the #03
boot-fault root-cause fix. See ../qwen38-dflash-v17/README.md for the v14
lineage (five-file overlay, supervisor, telemetry harness).

## Fix vs v17

| Issue | File(s) | Change | Env knob (default) |
|---|---|---|---|
| #03 boot-fault: XPU graphs + drafter | gpu_worker.py | v17's ENV enabled `VLLM_XPU_ENABLE_XPU_GRAPH=1` for the first time; with a dflash drafter the eager post-capture `_dummy_run` (max_num_reqs x (k+1) spec tokens — never-captured shape) faults the xe ccs engine 4/4 launches (a8/a9/a9r/a9r3: guc_id 24/38, ~60s post-capture, survives warm AND cold reboots; graphs-off+spec and graphs-on+nospec both healthy). Fix: extend the #05b post-capture skip to `spec_active AND VLLM_XPU_ENABLE_XPU_GRAPH=1` — synthesize hidden states for the sampler warmup; captured graphs already exercised the kernels | `VLLM_XPU_SPEC_SAFE_WARMUP` (1) |

## Fixes vs v14 (inherited from v17)
