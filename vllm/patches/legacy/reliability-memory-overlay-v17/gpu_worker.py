...
class Worker(WorkerBase):
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if self.use_v2_model_runner:
            ...
            # llm-scaler fix (#05b): with TurboQuant KV the eager
            # post-capture _dummy_run (max_num_reqs x spec tokens, a
            # never-captured shape) drives the TQ triton kernels on a
            # faulting path and wedges the XPU queue (v14 battery: k8v4
            # and k3v4_nc post-capture hangs). The captured graphs have
            # already exercised the kernels; only the sampler/logits
            # buffer warmup matters, so synthesize hidden states instead
            # of an eager forward. VLLM_XPU_TQ_SAFE_WARMUP=0 restores
            # upstream behavior.
            ...
            if hidden_size is not None:
                ...
                logger.warning(
                    "TurboQuant KV (%s): skipping eager post-capture "
                    "_dummy_run (#05b) - synthesized hidden states %s "
                    "for sampler warmup.",
                    kv_dtype,
