...
def _maybe_permute_muse_glimmer_qk_weight(
    config: Qwen3Config,
    name: str,
    loaded_weight: torch.Tensor,
) -> torch.Tensor:
    architectures = getattr(config, "architectures", None) or ()
    if not any("MuseGlimmerAssistant" in arch for arch in architectures):
    ...
    if name.endswith(".q_proj.weight"):
        ...
        return (
            loaded_weight.view(2, loaded_weight.shape[0] // 2)
            .transpose(0, 1)
    ...
    def __init__(
        ...
        *,
        ...
        if cache_config is None:
            # Inherit the KV cache dtype from the active vllm_config so the
            # drafter's KV cache spec follows --kv-cache-dtype (e.g. fp8).
            # Without this, Attention defaults to "auto" (model dtype), which
            # yields a 2x page size vs the target and breaks hybrid page-size
            # unification (MambaSpec pages cannot be rescaled).
...
class DFlashQwen3Model(nn.Module):
    ...
    def _build_fused_kv_buffers(self) -> None:
        ...
        self._context_chunk_size = 1024

    @staticmethod
    def _slice_context_slot_mapping(
        context_slot_mapping: torch.Tensor | Mapping[str, torch.Tensor] | None,
        start: int,
        end: int,
    ) -> torch.Tensor | Mapping[str, torch.Tensor] | None:
        if context_slot_mapping is None:
            return None
        if isinstance(context_slot_mapping, Mapping):
            return {
                layer_name: slot_mapping[start:end]
                for layer_name, slot_mapping in context_slot_mapping.items()
            }
    ...
    def precompute_and_store_context_kv(
        ...
        """Precompute context K/V in bounded chunks and write it to the cache."""
        num_ctx = context_states.shape[0]
        if num_ctx <= self._context_chunk_size:
            self._precompute_and_store_context_kv_chunk(
                context_states, context_positions, context_slot_mapping
            )
            return

        for start in range(0, num_ctx, self._context_chunk_size):
            end = min(start + self._context_chunk_size, num_ctx)
            self._precompute_and_store_context_kv_chunk(
                context_states[start:end],
                context_positions[start:end],
                self._slice_context_slot_mapping(context_slot_mapping, start, end),
            )

    def _precompute_and_store_context_kv_chunk(
        self,
        context_states: torch.Tensor,
        context_positions: torch.Tensor,
        context_slot_mapping: torch.Tensor | Mapping[str, torch.Tensor] | None = None,
...
class DFlashQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def compute_logits(
        ...
        # The remap indices are constant and the full-vocab scratch has a
        # stable upper bound, but this runs from Python on every spec-decode
        # step. Rebuilding them pumps ~3MB/step per rank through the XPU
        # caching allocator, which wedges the xe engines under PIECEWISE XPU
        # graphs once free memory runs low (same failure mode as the
        # TurboQuant decode scratch). Cache both grow-only instead.
        targets = getattr(self, "_logits_remap_targets", None)
        if targets is None or targets.device != logits.device:
            base = torch.arange(
                self.config.draft_vocab_size, device=logits.device
            )
            targets = base + self.draft_id_to_target_id
            self._logits_remap_targets = targets

        num_tokens = logits.shape[0]
        buf = getattr(self, "_logits_new_buf", None)
        layout_ok = (
            buf is not None
            and buf.shape[1] == self.config.vocab_size
            and buf.dtype == logits.dtype
            and buf.device == logits.device
        )
        if not layout_ok or buf.shape[0] < num_tokens:
            if buf is not None:
                # Keep replaced scratch alive: a captured graph may still
                # hold its address.
                self._retired_logits_bufs = getattr(
                    self, "_retired_logits_bufs", []
                )
                self._retired_logits_bufs.append(buf)
            capacity = max(num_tokens, buf.shape[0] * 2) if layout_ok else num_tokens
            buf = torch.empty(
                (capacity, self.config.vocab_size),
                dtype=logits.dtype,
                device=logits.device,
            )
            self._logits_new_buf = buf
        logits_new = buf[:num_tokens]
        logits_new.fill_(float("-inf"))
        logits_new.scatter_(1, targets.expand(num_tokens, -1), logits)
        return logits_new
...
class VanillaMarkov(nn.Module):
    """Low-rank learned bigram bias added to the draft logits (DSpark).

    Mirrors SpecForge's ``dspark.DSparkDraftModel`` head (adapted from
    DeepSeek DeepSpec): position i's logits are biased by a rank-``markov_rank``
    projection of the previous token's embedding.
    """
...
class AcceptRatePredictor(nn.Module):
    """Per-position acceptance-probability predictor (DSpark confidence head)."""
...
class DSparkDraftModel(DFlashQwen3ForCausalLM):
    """DSpark drafter: the DFlash backbone plus a Markov bigram-bias head and
    an optional per-position acceptance-confidence head.

    SpecForge-trained DSpark drafters (e.g. rwmacy/qwen3.8-27b DSpark draft
    checkpoints) carry the DFlash backbone weights plus
    ``markov_head.markov_w{1,2}.weight`` and, when the confidence head is
    enabled, ``confidence_head.proj.{weight,bias}``. The heads bias/score the
    full draft vocabulary, matching ``compute_logits`` output when
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.confidence_head: AcceptRatePredictor | None = None
        if getattr(config, "enable_confidence_head", False):
            ...
            if getattr(config, "confidence_head_with_markov", False):
                if self.markov_head is None:
                    raise ValueError(
                        "confidence_head_with_markov=True requires a Markov "
                        "head (markov_rank > 0)."
                    )
        ...
        if self.markov_head is not None:
            self.markov_head.to(dtype)
        if self.confidence_head is not None:
            self.confidence_head.to(dtype)

    def compute_markov_bias(
        self, prev_tokens: torch.Tensor
    ) -> torch.Tensor | None:
        """Full-vocabulary bigram bias for the given previous tokens."""
        if self.markov_head is None:
            return None
        return self.markov_head.compute_step_bias(prev_tokens)

    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        # The head weights are top-level modules of this class; everything
        # else follows the DFlash layout (backbone under ``model.``,
        # ``lm_head``, ``fc``).
        head_prefixes = ("markov_head.", "confidence_head.")
        head_weights: dict[str, torch.Tensor] = {}
        backbone: list[tuple[str, torch.Tensor]] = []
        for name, loaded_weight in weights:
            if name.startswith(head_prefixes):
                head_weights[name] = loaded_weight
            else:
                backbone.append((name, loaded_weight))
        super().load_weights(backbone)
        if head_weights:
            params_dict = dict(self.named_parameters())
            for name, loaded_weight in head_weights.items():
                param = params_dict[name]
