#!/bin/bash
# cargame_matrix.sh <CELL> - one car-game matrix cell on llm-scaler-vllm-adv:v20
# (host 10.20.3.59). One boot per cell; monitor3 telemetry; graceful teardown.
...
#   k2c1..k2c4                 spec k2   (v20 recommended arm: cheapest verify
#                                        steps; measured best true tok/s)
#   k6c3  turboquant_4bit_nc  spec k6   (k-curve reference)
# ACCEPT per KV: spec cell TRUE steady tok/s >= matching nospec cell.
set -u
CELL="${1:?usage: cargame_matrix.sh <bar|nbf16|nfp8|nk8v4|c1|c2|c3|c4|k2c1|k2c2|k2c3|k2c4|k6c3>}"
LOG=/root/telemetry/cargame_matrix.log
SERVELOG=/root/telemetry/serve_v20_${CELL}.log
ts() { date '+%F %T'; }
...
case "$CELL" in
  bar)   KV=turboquant_4bit_nc; SPEC=0; MAXLEN=262144; K=4 ;;
  nbf16) KV="";                  SPEC=0; MAXLEN=262144; K=4 ;;
  nfp8)  KV=fp8_e4m3;            SPEC=0; MAXLEN=262144; K=4 ;;
  nk8v4) KV=turboquant_k8v4;     SPEC=0; MAXLEN=262144; K=4 ;;
  c1)    KV="";                  SPEC=1; MAXLEN=73728;  K=4 ;;  # bf16 tgt ~8.4GiB + drafter
  c2)    KV=fp8_e4m3;            SPEC=1; MAXLEN=98304;  K=4 ;;
  c3)    KV=turboquant_4bit_nc;  SPEC=1; MAXLEN=98304;  K=4 ;;  # vLLM est. max 118784
  c4)    KV=turboquant_k8v4;     SPEC=1; MAXLEN=98304;  K=4 ;;
  k2c1)  KV="";                  SPEC=1; MAXLEN=73728;  K=2 ;;
  k2c2)  KV=fp8_e4m3;            SPEC=1; MAXLEN=98304;  K=2 ;;
  k2c3)  KV=turboquant_4bit_nc;  SPEC=1; MAXLEN=98304;  K=2 ;;
  k2c4)  KV=turboquant_k8v4;     SPEC=1; MAXLEN=98304;  K=2 ;;
  k6c3)  KV=turboquant_4bit_nc;  SPEC=1; MAXLEN=98304;  K=6 ;;
  k6c1)  KV="";                  SPEC=1; MAXLEN=73728;  K=6 ;;
...
cd /root/build/v20 || exit 4
echo "$(ts) cell=$CELL LAUNCH kv=[${KV:-bf16}] spec=$SPEC k=$K" >> "$LOG"
env TARGET_DIR=/models/qwen3.8-27b-fp8 \
    DRAFTER_DIR=/models/drafter-fp8-v5 \
    SPEC=$SPEC SPEC_K=$K KV_DTYPE="$KV" DTYPES=float16 BLOCKSIZE=128 MNBT=8192 MAXLEN=$MAXLEN \
