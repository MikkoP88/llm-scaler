...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        # llm-scaler fix (#05b): TurboQuant KV caches are incompatible
        # with the dflash/dspark drafter on XPU - acceptance collapses to
        # ~1-2% and the spec warmup path wedges the queue (v14 battery:
        # k8v4/k3v4_nc graphs hang). Auto-serve target-only instead of
        # hanging; opt back in with VLLM_ALLOW_TQ_SPEC=1.
        kv_cache_dtype = self.cache_config.cache_dtype
        if (
            self.speculative_config is not None
            and isinstance(kv_cache_dtype, str)
            and kv_cache_dtype.startswith("turboquant")
            and os.getenv("VLLM_ALLOW_TQ_SPEC", "0") != "1"
        ):
            logger.warning(
                "llm-scaler: speculative decoding auto-disabled - "
                "kv_cache_dtype=%s with a drafter is unsupported on XPU "
                "(KNOWN_ISSUES #05b). Serving target-only; set "
                "VLLM_ALLOW_TQ_SPEC=1 to override.",
                kv_cache_dtype,
            )
