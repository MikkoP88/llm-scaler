...
# llm-scaler v23/v24 (KNOWN_ISSUES #11): prefill-step peer rendezvous for the
# spec drafter. Resolved once at import; VLLM_SPEC_PREFILL_BARRIER=0 restores
# the v22 behavior (no barrier). See propose_draft_token_ids for the rationale.
_SPEC_PREFILL_PEER_BARRIER = (
    os.environ.get("VLLM_SPEC_PREFILL_BARRIER", "1") == "1"
)
# llm-scaler v24: the TP group's gloo cpu_group, resolved lazily on first use
# (the group does not exist when this module is imported). None = not yet
# resolved, False = unavailable (single rank / init failure).
_SPEC_PEER_RENDEZVOUS_GROUP = None


def _spec_peer_rendezvous_group():
    global _SPEC_PEER_RENDEZVOUS_GROUP
    if _SPEC_PEER_RENDEZVOUS_GROUP is None:
        try:
            from vllm.distributed import get_tp_group

            tp_group = get_tp_group()
            _SPEC_PEER_RENDEZVOUS_GROUP = (
                tp_group.cpu_group if tp_group.world_size > 1 else False
            )
        except Exception:
            _SPEC_PEER_RENDEZVOUS_GROUP = False
    return _SPEC_PEER_RENDEZVOUS_GROUP
...
class GPUModelRunner(
    ...
    def propose_draft_token_ids(
        ...
        # llm-scaler v24 (KNOWN_ISSUES #11): prefill-step PEER RENDEZVOUS for
        # the spec drafter. With TP>1 the drafter (MTP/EAGLE/dflash) issues
        # its own TP collectives (the head's vocab all-gather) immediately
        # after the large prefill forward. Wedge mechanism (py-spy, both
        # v22 and the failed v23 one-sided drain): the FASTER rank's host
        # runs ahead into the drafter and submits the all-gather; its
        # non-preemptible oneCCL spin-wait kernels then monopolize the
        # engines and starve that rank's own earlier in-flight collective
        # sends; the SLOWER rank's pending collective receives therefore
        # never complete, its stream never drains, its host blocks forever
        # in the align-mode .cpu() D2H submission (appendUSMMemcpy) BEFORE
        # it can join the all-gather - eternal spin, no reset, request
        # stuck Running until the client aborts. A one-sided
        # torch.xpu.synchronize() (v23) does NOT fix this: each rank drains
        # and immediately proceeds, so the fast rank still enters the
        # collective alone. The fix is a true host-level rendezvous: drain
        # this rank's stream, then meet the peer on the gloo cpu_group
        # (pure host barrier - no GPU kernels, cannot itself wedge) before
        # any drafter work is submitted. The fast rank now waits; the slow
        # rank's stream drains (its completion needs only sends the fast
        # rank already submitted), both ranks enter the drafter with
        # empty, symmetric queues. Decode steps (max_num_seqs * (k+1)
        # tokens - 320 at 64 seqs / k=4) skip the rendezvous and keep the
        # async-scheduling fast path.
        if _SPEC_PREFILL_PEER_BARRIER and num_scheduled_tokens >= 512:
            torch.xpu.synchronize()
            _peer_group = _spec_peer_rendezvous_group()
            if _peer_group is not False:
                # true host-level rendezvous (gloo cpu_group, no GPU kernels)
                torch.distributed.barrier(group=_peer_group)
            # else: no cpu_group (single rank / not initialized) - the
            # one-sided drain above is all we can do.
